...
Клон рядом с проектами, и он остаётся на виду: на его путь смотрит импорт
правил в каждом проектном файле. `update` доводит остальное сам, и тулчейна ему
не нужно: переводит клон на новейший релизный тег, качает тарболл своей
платформы, сверяет сумму, кладёт бинари утилит и обёртку `devkitctl` в каталог
назначения (`~/go/bin`, если он уже в PATH, иначе `~/.local/bin`) и
