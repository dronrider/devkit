...
- `hooks/` - хуки харнеса и git. Проверки текстов (запрещённые символы, индекс
  ...
  `agent-watch.py` говорит той же вестью внутрь сессии, чтобы итог фоновой
  работы, отчёт субагента или код возврата команды, не потерялся по дороге, `quota-refresh.sh` освежает снимок квоты на
  старте сессии, а `check-review.py` не пускает пуш и создание MR, пока у кода
  нет следа ревью, см. [hooks/README.md](hooks/README.md).
