...
одно: коммит, зашитый в бинарь, равен коммиту HEAD. Любое расхождение это
находка `doctor`, потому что бинари и правила составляют одну версию. Отсюда
...
отвязанном чекауте не работает, и напоминанием служит находка про давний поход
за тегами (mtime `FETCH_HEAD` старше двух недель). Спросить самому это
`devkitctl update --check`, он ходит за тегами и на машине ничего не меняет.
...
Механику везут задачи цели DK-138: версию и релизную сборку DK-150, установку и
обновление DK-151, сверку версий доктором DK-152. До них установка описана
старым порядком, с циклом `go build`.
