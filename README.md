...
- `kit/skills/` - процедуры, общие всем проектам, по директории с `SKILL.md` на
  ...
  идут несколько рук, `interview` ведёт разговор постановки с человеком одним
  порядком на все входы (раунд от перечня развилок, вопросы пачками по четыре,
  останов по краю без открытых человеческих развилок), `goal-start` ставит цель (диалог, бюджет, файл цели),
  ...
  `prose` держит корпус эталонов живой прозы (четыре жанра, форма и тип у
  фрагмента, словарь пользователя, сборщик реплик из журналов сессий) и кладёт
  в контекст письма горячий список из пяти примет и случайный набор фрагментов,
  а ключ `sample --form` ставит первым образец той формы, которая пишется
  (абзацы, список, таблица, шаги); зовут его
  оформление черновика, файл задачи, решение LLD и правка README,
  `prompt-test` проверяет правку самих промптов прогоном стенда `obeycheck`,
  `chat` ведёт разговор с человеком по задаче (ответ текстом в тот же
  разговор, подпись реплики человека в межсессионном канале
  `from-name="human"`, долгие дела субагенту), а хук `session-task.py` после
  сжатия контекста просит его перечитать. Первые три
  ...
  --fix`, только по каталогам `[skills] dir` включённых харнесов. Скилл, который
  зовёт только человек командой `/имя` (`goal-start`, `board-team`, `live-core`),
  несёт во frontmatter `disable-model-invocation: true`: харнес не кладёт его
  описание в реестр модели, и в резиденте он не стоит ничего. Витки цели без
...
- `hooks/` - хуки харнеса и git. Проверки текстов (запрещённые символы, индекс
  ...
  событиям сессии: `notify.py` уведомляет громко о том, что сессия ждёт действия
  или закончила ход, и фоново о том, что субагент отработал, `agent-watch.py`
  говорит той же вестью внутрь сессии, чтобы итог фоновой работы, отчёт
  субагента или код возврата команды, не потерялся по дороге, `quota-refresh.sh`
  освежает снимок квоты на старте сессии, а `check-review.py` не пускает пуш и
  создание MR, пока у кода нет следа ревью, см.
...
`commit-msg`, `pre-push`), хуки старта сессии `hooks/quota-refresh.sh`,
`hooks/board-catchup.sh` и `hooks/devkit-catchup.sh` и фикстуры в `testdata/`, которые изображают чужую
...
Ревью правки любого происхождения идёт по скиллу `review` в
`kit/skills/review/`. Предмет бывает трёх видов. Задача доски перед
слиянием, MR коллеги в GitLab, Gitea или GitHub, дифф ветки без трекера.
Скилл читает агент. Человеку остаются три шага: разложить, настроить,
позвать.

1. Разложить. `devkitctl doctor --fix` из клона devkit кладёт скилл с
   `examples.md` и `threads.md`, определения ревьюверов и хуки в каталоги
   харнеса. Без Go на машине бинари утилит приезжают только релизом через
   `devkitctl update`.
2. Настроить. Файл `.devkit/review.conf` лежит в корне проекта, у
   корп-проекта в боковой директории рядом с клоном. Формат `ключ =
   значение`, `#` комментарий. Образец лежит в `.devkit/review.conf` самого
   devkit. Без файла действуют умолчания из таблицы.
3. Позвать. Ревью задачи доски зовёт диспетчер по `board-ship` перед
   слиянием. Чужой MR идёт по просьбе «проведи ревью задачи AB-123» либо
   сразу командой `trackctl review AB-123`. Дальше `taskctl review draft`,
   `publish`, `poll` и `approve-mr` по разделу «Ревью чужой задачи» скилла.

| Ключ | Умолчание | Что задаёт |
|---|---|---|
| `level1`, `level2`, `level3` | `5 минут, 20 ходов`; `20 минут, 70 ходов`; `40 минут, 100 ходов` | бюджет первого круга по уровням тщательности |
| `retry` | `1/3` | доля бюджета на второй круг |
| `rounds` | `2` | потолок кругов, дальше вопрос человеку |
| `critical_paths` | пусто | пути проекта, совпадение файла диффа даёт «да» в триаже критичности |
| `labels` | `issue, suggestion (non-blocking)` | метки тредов человеку |
| `checks` | пусто | проектные вопросы ревью, строка «уровень: вопрос» |
| `publish` | `confirm` | публикация замечаний в чужой MR и апрув: `confirm` после слова человека, `auto` сам |
| `pause` | `20-60` | пауза между публикациями тредов, секунды числом или вилкой |
| `poll` | `5m` | шаг опроса тредов тиком у строк «ждёт автора» |
| `silence` | `1d` | порог молчания автора, дальше одно уведомление человеку |
| `worklog` | `off` | ворклог времени ревью в тикет коллеги при закрытии строки |

Первое ревью коллеги стоит гонять в `confirm`. Замечания ложатся в
`docs/tasks/<ID>.review.md`, вы читаете файл и говорите «публикуй». На `auto`
переходят руками после нескольких таких файлов. Бюджеты пересчитывает
`taskctl review stats` по накопленным ревью. Команды тредов и образцы
разделов лежат рядом со скиллом, свод ключей с разбором в
[tools/taskctl/README.md](tools/taskctl/README.md).

## Подъём работы после смерти сессий

Перезагрузка машины убивает все сессии разом. Гибнут панели tmux, заходы
харнеса и фоновые прогоны. Службы после старта поднимаются сами, а работа
стоит. Поднимает её `devkitctl lift`.

Первый запуск лучше сделать сухим. Он ничего не трогает.

```
devkitctl lift --dry-run
```

Команда обходит корни под надзором и печатает находки. Это строки, чей этап
означает работу, а живой сессии за ними нет. Рядом идут замки задач с мёртвым
владельцем, сиротские процессы прогонов и брошенные временные деревья. Без
ключа тот же заход поднимает и убирает.

```
devkitctl lift
```

Строка поднимается обычным заказом `taskctl run`. Реплика заказа называет саму
строку, и чужую работу с доски поднятая сессия не берёт. Подъём идёт по одной
и в пределах свободной ёмкости. Потолок корня берётся у `agentctl budget`, из
него вычитаются строки с живыми сессиями. Поверх стоит сквозной потолок на
машину. Ключ `-C` сужает заход до одного корня.

Только что поднятая строка пятнадцать минут не поднимается второй раз. Сессия
заводит транскрипт не мгновенно, и до первой записи доска говорит «сессии
нет». Строка, чья сессия не удержалась три раза подряд, дальше ждёт человека.
Заход о ней так и пишет. След лежит в `~/.devkit/lift.log` и забывается через
сутки.

Ту же механику зовёт тик сторожка `ru.devkit.goal-watch`. После перезагрузки
работа поднимается без участия человека, а отчёт захода целиком уходит в
