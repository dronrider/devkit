...
- `trackctl/` - Go-утилита корп-контура: всё, что говорит с трекером компании
  (status/issue/take), собрано в ней, а taskctl остаётся механикой доски и в
  сеть не ходит. Трекер прибит контрактом адаптера, статусы компании
...
- `skills/` - процедуры, общие всем проектам, по директории с `SKILL.md` на
  ...
  задач доски от отбора кандидатов до выката и проверки, `board-groom` оформляет
  черновики `taskctl draft` в строки Backlog, `goal-start` ставит цель (диалог,
  бюджет, файл цели), `goal-loop` ведёт её витки над обычным конвейером,
