...
- `kit/skills/` - процедуры, общие всем проектам, по директории с `SKILL.md` на
  ...
  задач доски от отбора кандидатов до выката и проверки, `board-groom` разбирает
  ...
  `goal-loop` ведёт её витки над обычным конвейером, `live-core` правит сам
  devkit по ходу цели, когда его недостаток мешает витку. Первые три это
  процедурная часть правил: резидентно только ядро, а шаги приезжают по вызову
