...
Перезагрузка машины убивает все сессии разом: панели tmux, заходы харнеса и
фоновые прогоны. Службы после старта поднимаются сами, а работа стоит, потому
что поднимать её было некому. Этим занимается `devkitctl lift`.
...
Команда обходит корни под надзором и печатает находки: строки, чей этап
означает работу, а живой сессии за ними нет, замки задач с мёртвым владельцем,
сиротские процессы прогонов и брошенные временные деревья. Без ключа тот же
заход поднимает и убирает:

```
devkitctl lift
```

Строка поднимается обычным заказом `taskctl run` с репликой, которая называет
саму строку. Подъём идёт по одной и в пределах свободной ёмкости: потолок
корня берётся у `agentctl budget`, из него вычитаются строки с живыми сессиями,
а поверх стоит сквозной потолок на машину. Ключ `-C` сужает заход до одного
корня.
...
нет». Строка, чья сессия не удержалась три раза подряд, дальше ждёт человека, и
заход о ней так и пишет. След лежит в `~/.devkit/lift.log` и забывается через
сутки.

Ту же механику зовёт тик сторожка `ru.devkit.goal-watch`, поэтому после
перезагрузки работа поднимается без участия человека. Отчёт захода целиком
уходит в `~/.devkit/goal-watch.log`.
