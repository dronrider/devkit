...
- `kit/` - материал агентской сессии: определения субагентов (`agents/`),
  ...
  проект или читает как данные; кода тут нет, кроме оболочки скилла, которая
  лежит рядом со своим `SKILL.md`.
...
`AGENTS.md`, `CLAUDE.md`), и правила (`RULES*.md`, `RANKING.md`). Правила
остаются в корне по той же причине, что и хуки: на `RULES.core.md` ссылается
глобальный `~/.claude/CLAUDE.md` и тонкий файл каждого подключённого проекта.
Исполняемых файлов в корне нет, и нового каталога верхнего уровня без решения
в LLD не заводится.
...
которые изображают чужую программу. На `docs/tasks/` и на `testdata/` правило
языка не смотрит: сценарий проверки пишется под один прогон и после закрытия
задачи не запускается, а фикстуре язык диктует не наше правило.
