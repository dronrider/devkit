...
- `skills/` - процедуры, общие всем проектам, по директории с `SKILL.md` на
  ...
  черновики `taskctl draft` в строки Backlog, `goal-loop` держит режим цели
  (постановка с бюджетом и цикл витков над обычным конвейером), `live-core`
  правит сам devkit по ходу цели, когда его недостаток мешает витку. Первые три
