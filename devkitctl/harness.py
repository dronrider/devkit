...
"""Парсер и валидатор профилей харнесов (подмножество TOML).
  ...
  python3 harness.py <файл>...
      ...
      profile-*.toml и <харнес>.toml из harness/ вдобавок валидируются сводом
...
harness/testdata: один и тот же вход обе стороны обязаны читать одинаково.
"""
import os
import sys
...
def report(name, text, validate=True):
    ...
    # с предупреждениями. Им сверяются фикстуры harness/testdata.
    try:
        d = parse(name, text)
    except TomlError as e:
        return "error: %s\n" % e
