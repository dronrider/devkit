...
|--------|--------|-----|---|---|------|--------|
| DK-066 | Два исхода проверки: провал выката и приёмка с замечаниями, непроверенный выкат виден после возврата в работу | task | P2 | 36 (25+6+3+0+2) | M | [tasks/DK-066.md](tasks/DK-066.md) |
| DK-051 | agentctl budget: размер пачки из остатка лимитов | task | P3 | 13 (0+7+1+0+5) | S | [tasks/DK-051.md](tasks/DK-051.md) |
...
| ID | Задача | Тип | P | R | Цена | Ссылка |
|--------|--------|-----|---|---|------|--------|
| DK-069 | Уведомитель шумит из песочницы: живой баннер на taskctl move во временном репозитории | bug | P2 | 38 (25+4+2+5+2) | S | [tasks/DK-069.md](tasks/DK-069.md) |
...
| DK-054 | скилл board-batch, раскладка скиллов и ключ test в devkitctl [после DK-051, DK-052, DK-053] | task | P3 | 12 (0+6+1+0+5) | M | [lld/DK-048-batch-dispatcher.md](lld/DK-048-batch-dispatcher.md) |
...
| DK-070 | mergedShas читает файл задачи построчно и не знает про ограждённые блоки: процитированный раздел «Выкат» становится настоящей записью | bug | P3 | 9 (0+3+1+5+0) | S | [tasks/DK-070.md](tasks/DK-070.md) |
| DK-072 | devkitctl/test.sh без бита x, локальный прогон как в доке падает | bug | P3 | 9 (0+2+0+5+2) | S | - |
...
| DK-068 | Развести Blocked и зависимость [после ...]: внешнее обстоятельство против своей же задачи | task | P3 | 6 (0+3+2+0+1) | S | [tasks/DK-068.md](tasks/DK-068.md) |
