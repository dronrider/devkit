...
| ID | Задача | Тип | P | R | Цена | Ссылка |
|--------|--------|-----|---|---|------|--------|
| DK-010 | LLD: квотный корректор вердикта pick при недоступном остатке лимитов подписки | LLD | P3 | 12 (0+5+5+0+2) | - | - |
| DK-009 | agentctl: fable-ярус маппинга (LLD по цене, сложный код) и доменный override модели | task | P3 | 11 (0+5+3+0+3) | M | - |
| DK-011 | agentctl: effort в вердикте pick и применение через определения субагентов | task | P3 | 10 (0+4+3+0+3) | M | - |
