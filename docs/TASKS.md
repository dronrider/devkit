...
| DK-041 | хуки: разбор входа по имени протокола, следы ходовых ассистентов в check-commit | task | P3 | 14 (0+8+1+0+5) | S | [tasks/DK-041.md](tasks/DK-041.md) |
...
| DK-056 | Неопределённость в RANKING.md растёт от пересекаемых границ: правка через процессы и языки (сервер плюс клиент, JNI, UI) это минимум 3 | task | P3 | 9 (0+4+1+0+4) | S | [tasks/DK-056.md](tasks/DK-056.md) |
...
| DK-072 | devkitctl/test.sh без бита x, локальный прогон как в доке падает | bug | P3 | 9 (0+2+0+5+2) | S | [tasks/DK-072.md](tasks/DK-072.md) |
