...
|--------|--------|-----|---|---|------|--------|
| DK-157 | Лог установки говорит по-человечески, а окружение доезжает без ручных brew [провал: живой прогон установки: список «осталось сделать» не свёрнут (семь однотипных строк про PATH по числу утилит плюс общая восьмая), а пакет, который менеджер поставить не смог, описан враньём с пустыми скобками: «brew install tmux не прошёл ()», хотя команда отработала нулём] | task | P2 | 36 (25+7+1+0+3) | M | [tasks/DK-157.md](tasks/DK-157.md) |

## Check (готово, ждёт проверки пользователем)

| ID | Задача | Тип | P | R | Цена | Ссылка |
|--------|--------|-----|---|---|------|--------|
| DK-138 | Цель: devkit ставится на чистую машину без сборки из исходников | task | P2 | 40 (25+7+4+0+4) | XL | [tasks/DK-138.md](tasks/DK-138.md) |

## Backlog

| ID | Задача | Тип | P | R | Цена | Ссылка |
|--------|--------|-----|---|---|------|--------|
| DK-156 | Цель: автономный цикл не стоит молча и переживает обрывы | task | P1 | 65 (50+8+3+0+4) | XL | [tasks/DK-156.md](tasks/DK-156.md) |
...
| DK-153 | Сторожок цикла цели: простой дольше порога зовёт громко | task | P1 | 63 (50+7+3+0+3) | M | [tasks/DK-153.md](tasks/DK-153.md) |
...
| DK-154 | shipctl: вставший deploy проваливается по пределу, ожидание фона через Monitor | task | P2 | 34 (25+5+2+0+2) | M | [tasks/DK-154.md](tasks/DK-154.md) |
