...
| DK-110 | devkitctl weigh: живой замер резидента headless-прогоном и сверка с расчётом doctor [провал: замер занижен: импорт правил путём наружу клиент не разворачивает, RULES.board.md не доезжает до целевого прогона] | task | P3 | 13 (0+6+3+0+4) | M | [tasks/DK-110.md](tasks/DK-110.md) |
