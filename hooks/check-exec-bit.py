#!/usr/bin/env python3
"""Проверка бита исполнения у test.sh по всему репозиторию: `devkitctl/README.md`
и подобные доки показывают прогон вида `devkitctl/test.sh` напрямую, а без
...
def main(argv):
    ...
    findings = [
        ...
        for path, mode in tracked_modes(repo, patterns)
        if mode != WANT_MODE
    ]
    for f in findings:
        print(f)
