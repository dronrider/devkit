...
# с заглавной буквы, словарь из восьми этапов и трёх ожиданий (DK-911) плюс
# два слова прежнего словаря, которые остались в старых пакетах. Заметку
# строят писатели этапа: хук спавна субагента (hooks/stagerun.py), shipctl,
# taskctl и agentctl. Формат общий для всех, и разбирать их разными
# регулярками смысла нет. Перечень зеркалит internal/stage.Kinds, расхождение
# ловит TestDictionaryMirrorsMatchGo в internal/stage.
STAGE_LABELS = ("Постановка", "Разработка", "Вычитка", "Ревью", "Доработка",
                "Слияние", "Выкат", "Проверка", "Ждёт человека", "Ждёт события",
                "Ждёт очереди", "Снаружи", "Уточнение")
...
# замечаний.» либо, с DK-812, «- Вердикт: без замечаний до <sha>.»
# (cleanVerdictHead, cleanVerdictHeadFmt). Хвост после головы пишет
# ревьювер, и это суть проверки не хуже отдельного замечания: голову счёт
# метрик режет, хвост остаётся под ним, как и раньше. Цену обратного
# показал DK-1050: ревьювер руками убрал двоеточие из головы вердикта,
# спасаясь от сторожа, и ворота слияния перестали узнавать чистый исход.
REVIEW_LEVEL_HEAD_RE = re.compile(r"^Уровень [0-3] до \S+:\s*")
REVIEW_VERDICT_HEAD_RE = re.compile(
    r"^(-\s+)Вердикт:\s+без замечаний(?:\s+до\s+[0-9a-fA-F]{4,40})?\.\s*")
...
def strip_machine_head(line):
    ...
    «Уровень N до sha:» и «Вердикт: без замечаний.» (либо, с DK-812,
    «Вердикт: без замечаний до <sha>.») несут за собой суть проверки
    ревьювера, и её сторож меряет как прежде (DK-887): режется только
    голова, а не строка целиком.
    """
...
# Порог берётся перестройкой фразы, а не подгонкой смысла под счётчик. Случай
# 8421ff09 это находка в режиме warn, после которой агент переписал само
# утверждение (LLD DK-606, решение 2). Инвариант тот же, что у вычитки в
# разделе «Что не трогает». Повторён он тут потому, что находка приходит
# посреди чужого захода, где скилла в контексте нет.
LIMIT_RULE = (
    "порог берётся перестройкой фразы с тем же утверждением. Слова внутри "
    "фразы меняются, а предмет фразы остаётся прежним, и подгонять смысл "
    "под порог нельзя. Перестройка не вышла, тогда фраза остаётся как есть, а "
    "пометка пишется строкой в раздел «Ход работы» файла задачи. У текста "
    "вне задачи та же строка идёт в отчёт захода:")


def mark_of(f, where):
    """Пометка про сознательно оставленный порог, строкой в «Ход работы»."""
    return "- Сторож прозы: %s, %s %s при пороге %d, оставлено: <причина>." % (
        where or "?", f.metric.title, number(f.value), f.limit)
...
def report(found, where, words):
    ...
    marks = "\n".join(mark_of(f, where) for f in found)
    return "%s\n%s\nкак переписать:\n%s\n%s\n%s\n" % (
        head, body, how, LIMIT_RULE, marks)
