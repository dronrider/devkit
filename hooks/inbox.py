#!/usr/bin/env python3
"""Почтальон devkit: донести реплику человека до идущей сессии.

Реплику с экрана чата дашборд кладёт строкой в ящик, и до этого хука её читал
только следующий виток цели. Между двумя витками проходят десятки минут, а
человек, написавший «стой, не туда», всё это время ждёт. Почтальон стоит на
завершённом ходе инструмента, читает почту сессии и вносит лежащие строки
прямо в контекст идущего захода каналом добавки. Решение целиком в
docs/lld/DK-136-live-chat.md, раздел «Устройство почты».

Носителя два, и механика у них одна. Письма цели лежат в разделе «Входящие»
файла цели, адрес ищется через реестр целей (LLD DK-136). Письма любой другой
живой сессии лежат в ящиках .devkit/mail/<имя>.inbox дерева работы, и адресат
...
ящиков устойчивы к смене сессии: task-<ID> переживает парковку задачи, а
sess-<ID> это личный ящик сессии без задачи. Отметка доставки ящику не нужна:
доставленная строка уходит из него тем же замком, что и пишется, поэтому
лежащая строка всегда непрочитанная. Признак ожидания .ask и замок .lock у
ящика те же по формату, что у цели, и читаются теми же функциями.
...
Адрес писем цели ищется в три шага. Реестр целей ~/.devkit/goals/*.watch
...
session в каталоге замка, пока замок держит живая оболочка. Адрес писем ящика
проще: дерево это ближайший предок cwd с .git, а сессию называет сама строка
письма. Ход субагента не получает почты вовсе: реплика адресована витку или
...
# Подпись дашборда в строке письма: по ней из строки достаётся текст реплики.
# Рукописная строка идёт в журнал цикла как есть.
FROM_DASHBOARD = ", из дашборда: "
# Адресат внутри строки письма: «<стамп>, сессии <ID>, из дашборда: текст».
...
TO_SESSION = ", сессии "
# Каталог ящиков любой сессии в дереве работы: .devkit/mail/<имя>.inbox.
MAIL_DIR = "mail"
...
def log(session, addr, reason):
    ...
    либо имя ящика, поводы перечислены поимённо в README, и все они за
    развилкой «дерево сошлось с адресом почты», так что строк тут ровно
    столько, сколько на машине живых целей и ящиков с письмами."""
...
def write_lines(path, lines):
    """Переписать ящик оставшимися строками. Временный файл с заменой ради
...
def ask_stamp(path):
    """Срок признака ожидания из файла. Формат один у цели и у ящиков
    .devkit/mail: строка со штампом, и лежит срок внутри файла затем, чтобы
    убитый ход ожидания не запер ящик навсегда. None значит, что файла нет или
...
def ask_until(root, goal):
    ...
    ответа сам, вторым читателем почтальону там делать нечего."""
...
def tree_root(cwd):
    ...
    чужую почту. None значит, что ход идёт вне git-дерева, ящиков таким
    деревьям не заводят."""
...
def box_names(root):
    """Имена ящиков дерева: каталог .devkit/mail, файл <имя>.inbox. Ящик без
    писем убирается при доставке, поэтому живой файл в каталоге значит
    лежащее письмо."""
    d = os.path.join(root, ".devkit", MAIL_DIR)
    try:
        names = sorted(os.listdir(d))
    except OSError:
        return []
    cut = -len(".inbox")
    return [n[:cut] for n in names if n.endswith(".inbox")]


def addressee(line):
    """Сессия, которой письмо адресовано. Пустая строка значит «любой сессии
...
def box_letter(name, lines):
    """Текст добавки контекста из ящика. Отсылки к скиллу goal-loop тут нет:
    читатель ящика бывает исполнителем задачи и грумером черновика, и разряд
    реплики он выбирает по месту своей работы."""
    body = "\n".join("- %s" % said(line) for line in lines)
    return ("Живая реплика человека из ящика %s, доставлена посреди хода каналом чата devkit:\n"
            "%s\n"
            "Строка уходит из ящика в момент доставки, повторно она не приедет." % (name, body))


def serve_box(root, name, turn, now):
    """Почта одного ящика: текст добавки либо None. Отличие от цели в том, что
    доставленная строка уходит из ящика тут же, под тем же замком: читателя,
    который забирает строку своей записью, у ящика нет, и отдельная механика
    отметок ему не нужна. Отметкой служит отсутствие строки, и любые читатели,
    почтальон и будущий инструмент ожидания, расходят письма одним замком."""
    d = os.path.join(root, ".devkit", MAIL_DIR)
    box = os.path.join(d, name + ".inbox")
    until = ask_stamp(os.path.join(d, name + ".ask"))
    if until is not None and until > now:
        log(turn.session, name, "отказ: ящик держит вопрос до %s, ждёт инструмент ожидания"
            % time.strftime(STAMP, time.localtime(until)))
        return None
    fd = take_mail_lock(os.path.join(d, name + ".lock"))
    if fd is None:
        return None
    try:
        try:
        ...
        if not taken:
            if tracing() and lines:
                ...
                log(turn.session, name, "отказ: письмо адресовано сессии %s, а ход идёт сессией %s"
        ...
        try:
            if len(taken) == len(lines):
                ...
                write_lines(box, [l for l in lines if l not in lying])
        except OSError as e:
            log(turn.session, name, "отказ: доставленное из ящика не убрать: %s" % e)
            return None
    finally:
        os.close(fd)
    log(turn.session, name, "доставлено строк %d, первая «%s»"
        % (len(taken), short(said(taken[0]))))
    return box_letter(name, taken)
...
def run_hook(protocol, now=None):
    ...
    if root is not None:
        for name in box_names(root):
            text = serve_box(root, name, turn, now)
            if text:
                letters.append(text)
    if not letters:
        return 0
