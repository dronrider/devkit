...
# велеть некому. По этому плану дашборд рисует деления кольца в шапке
# разговора и блок «План агента» на экране задачи.
PLAN_RULE = ("Веди план работ инструментом TodoWrite: список этапов до первого шага, "
...
def task_context(task, cwd):
    ...
    if not root:
        return ""
    parts = ["Эта сессия открыта разговором про задачу %s." % task, PLAN_RULE]
    try:
