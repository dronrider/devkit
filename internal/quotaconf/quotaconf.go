// Package quotaconf читает настройку порога свежести снимка квоты. Она
// задаётся строкой `stale = <минуты>` в машинном файле
// ~/.devkit/quota.local. Порог решает, когда снимок считается протухшим.
// Держат его трое: agentctl (дорога кеш/панель, режим --if-stale и подпись
// возраста), демон дашборда (пометка свежести на экране) и через agentctl
// тик сторожка. Источник у всех один, этот пакет, копий константы нет
// (DK-633).
//
// Без файла или строки действует умолчание в 45 минут. Кривое значение это
// отказ с причиной. Порог, тихо съехавший на 45 минут из-за опечатки,
// разъехался бы с ожиданием человека и не был бы виден ничем.
package quotaconf
...
// не двигает вверх, а периодический съём переснимает.
