...
type Bind struct {
	...
	// Пусто у разговора, «цикл цели <ID>» у сессии, ведущей цикл (оболочка
	// витка либо заход по строке цели), у безголового захода слово из
	// DEVKIT_HEADLESS («дашборд», «taskctl run»). Без поля безголовый заход и
	// разговор человека по реестру неотличимы, а свод расхода относит их к
	// разным статьям (DK-913). Носитель цикла переживает саму запись цикла
	// в ~/.devkit/goals, и потому статья «фон» сходится и назавтра (DK-1088).
	Carrier string
...
// Слова источника целиком. «Заказ» это повод, которым сессию поднял хук
// старта, и он один на конвейер, чат по строке и разбор черновика: критерий
// работы у него свой, разбор в workSrc ниже. Работой считаются «дерево» (сессия
// стартовала в боковом дереве задачи) и «работа» (она двигала строку командой
// доски). «Рука» это привязка человеком с экрана, и работой она не считается:
// человек говорит, о чём разговор, а не что по задаче идёт правка. «Снята»
// отвязывает.
const (
	ByOrder = "заказ"
	ByTree  = "дерево"
