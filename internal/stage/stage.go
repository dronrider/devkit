...
// (LLD DK-503, решение 1): потолок захода меряется временем, а не числом
// обращений или объёмом контекста. Ищет от конца записи, а не берёт только
// живой этап, потому что вызывающий (taskctl elapsed) спрашивает конкретный
// вид деятельности, а живым к моменту вопроса может стоять уже другой этап.
// Второе значение false, если этапа такого вида в записи нет вовсе.
