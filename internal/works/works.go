...
// Sessions отдаёт сессии tmux машины. Ошибка это «спросить не удалось»: tmux
// не уложился в срок или ответил незнакомым отказом. Такой ответ не пустой
// список, и вызывающий его не применяет. Сторож дашборда пропускает обход, а
// планировщик слота не считает занятую задачу свободной (DK-904). Машина без
// tmux отвечает своим ErrNoTmux, и он к сорванному опросу не относится. Ненулевой код ls со словами про отсутствие сервера это штатное
