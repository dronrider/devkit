...
работе сессии, ждёт её конца. Фоновой работой тут зовутся и субагенты, и
запущенные оболочкой команды: реестр сторожа держит те и другие. В первом
случае оболочка встаёт по-хорошему, во втором заказа не подаёт и ждёт
следующего конца хода. Голова просыпается по концу субагента и по концу
команды, и поднимает её ход харнес, а не оболочка.
...
# там вперемешку, а ход головы поднимает конец и той, и другой, поэтому живая
# работа обоих разрядов придерживает проход. Разбираются они порознь: в журнал
# субагент идёт видом, а команда командной строкой.
AGENT_SUBAGENT = "subagent"
AGENT_SHELL = "shell"
# Сколько букв командной строки берёт журнал. Длинная команда с путями заняла
# бы строку целиком, а узнаётся она по началу.
SHELL_CUT = 60
...
class Pipeline:
    ...
    def busy(self):
        """Виды живых субагентов своей сессии. Разряда у записей, сделанных до
        DK-571, нет вовсе, и все они про субагентов."""
        return [str(v.get("type") or "-") for v in self.running(AGENT_SUBAGENT)]

    def shells(self):
        """Командные строки живых фоновых команд своей сессии. Имени человек
        команде не даёт, и узнаётся она по началу строки."""
        out = []
    ...
    def run_live(self):
        ...
        while True:
            ...
            # уходит ждать конца, ход при этом короток и строка стоит, а
