...
Проход кончается концом хода, каким бы он ни был. Конец нужен свой. Проход
закрывает та отметка, перед которой в нём лежит «начат». У хода второй реплики
на жалобу стоп-хука пары нет, и оболочка называет такую отметку в журнале и
ждёт дальше (DK-966). Ни код возврата, ни слово отметки тут не вердикт, вердикт
это статус строки на доске. Закрытая задача
...
встаёт. Трёх ожиданий она воронкой не считает, а после честного ожидания счёт
холостых начинается заново. Задача с видом приёмки mixed или user стоит в check
и ждёт человека, а проход, кончившийся при живой фоновой
работе сессии, ждёт её конца. Фоновой работой тут считаются и субагенты, и
запущенные оболочкой команды. Реестр сторожа хранит записи о тех и о других. В
первом случае оболочка встаёт по-хорошему, во втором не подаёт заказа и ждёт
...
MIXED_KIND = "mixed"
# Слова taskctl про отметку прогона в той же пометке (checkMarkLabel). Строка
# mixed без отметки ждёт не человека, а проверяющего: агентскую половину
# сценария ещё никто не гонял, и очередь выката держит она (DK-947). Своего
# разбора файла задачи у оболочки нет нарочно: форма отметки живёт в общем
# модуле (internal/taskform), и вторая копия разошлась бы с первой.
SMOKE_NONE = "без отметки smoke"
# Начало заказа проверяющему (internal/checkrun, OrderHead). По нему оболочка
# узнаёт собственную голову проверки: сдавать строку она никому не должна, это
# её работа.
CHECK_ORDER = "Прогони агентскую часть сценария проверки "
...
# Разряды записи реестра. Субагент и фоновая команда оболочки (DK-571) лежат
# там вперемешку, а ход головы поднимает конец и той, и другой. Живая работа
# обоих разрядов придерживает проход. Они разбираются порознь. В журнал
# субагент идёт видом, а команда командной строкой.
AGENT_SUBAGENT = "subagent"
AGENT_SHELL = "shell"
# Сколько букв командной строки попадает в журнал. Длинная команда с путями
# заняла бы строку целиком, а она узнаётся по началу.
SHELL_CUT = 60
...
class Pipeline:
    ...
    def check_kind(self, said, sect):
        ...
        заголовка сюда не попадёт. У строки не из check вида нет."""
        if sect != "check":
            return ""
    ...
    def waits_user(self, said, sect):
        ...
        Смешанный вид без отметки smoke сюда не идёт (DK-947): такую строку
        ждёт не человек, а проверяющий, и стоп «ждёт приёмки человеком» на ней
        останавливал конвейер там, где машине оставалась работа."""
        kind = self.check_kind(said, sect)
    ...
    def checker(self):
        ...
        return (self.order or "").startswith(CHECK_ORDER)

    def hands_over(self, said, sect):
        """Сдаётся ли строка проверяющему: вид mixed, выкат без отметки smoke, а
        голова тут не проверяющий. Своя голова проверки идёт дальше обычными
        проходами: отметку ставит она, и сдавать ей строку некому."""
        return (self.check_kind(said, sect) == MIXED_KIND
                and SMOKE_NONE in (said or "") and not self.checker())

    def hand_over(self, sect):
        """Сдать строку проверяющему и выйти без зова человека (DK-947).

        Агентскую половину сценария гоняет не автор правки, и поднимает этого
        проверяющего выкат либо тик сторожка командой `shipctl check-run`
        ярусом ревью, а при совпадении моделей ступенью выше. Ждать его в окне
        оболочка не берётся нарочно: замок головы задачи один на ID, и
        оставшаяся в окне оболочка отбивала бы подъём занятым замком. Работа
        головы к этому моменту кончена, очередь выката держит отметка, и
        конвейер за строкой не стоит."""
        self.stop(0, sect, "строка сдана проверяющему: вид mixed, отметки smoke нет, "
                           "агентскую половину сценария гоняет не автор правки "
                           "(подъём ведут выкат и тик, shipctl check-run)", loud=False)
    ...
    def running(self, job):
        """Живые работы своей сессии названного разряда по реестру сторожа
        (hooks/agent-watch.py). Пустой список значит, что фоновой работы нет.
        Нечитаемый реестр тоже считается пустым. Тут нечего гадать, а долгое
        молчание головы после такого прохода это mute."""
        if not self.sid:
            return []
        ...
        if not isinstance(got, dict):
            return []
        return [v for v in got.values()
                if isinstance(v, dict) and v.get("state") == AGENT_RUNNING
                and (v.get("job") or AGENT_SUBAGENT) == job]

    def busy(self):
        """Виды живых субагентов своей сессии. У записей, сделанных до
        DK-571, нет разряда вовсе, и все они про субагентов."""
        return [str(v.get("type") or "-") for v in self.running(AGENT_SUBAGENT)]

    def shells(self):
        """Командные строки живых фоновых команд своей сессии. Человек не
        даёт команде имени, и она узнаётся по началу строки."""
        out = []
        for v in self.running(AGENT_SHELL):
            said = str(v.get("command") or v.get("description") or "-").strip() or "-"
            said = said if len(said) <= SHELL_CUT else said[:SHELL_CUT] + "..."
            out.append(" ".join(said.split()))
        return out
    ...
    def wait_turn(self, n):
        ...
        Проход кончает только та отметка конца, перед которой в очереди лежал
        «начат». Пара эта и отличает конец заказанного хода от лишней отметки:
        стоп-хук сторожа фоновых работ жалуется на непрочитанный отчёт, голова
        отвечает второй репликой, и ход кончается ещё раз, а «начат» у него
        один. Лишняя отметка доставалась следующему проходу, закрывала его за
        ноль секунд и вела в воронку холостых (DK-966). Ход, начатый репликой
        человека, пару имеет и проход кончает по-прежнему."""
        ...
        while True:
            ...
            for i, (word, why, when) in enumerate(got):
                ...
                if word == TURN_STARTED:
                    ...
                    # разницы нет, ход пошёл в обоих случаях. Второй ход внутри
                    # прохода значит, что прошлый вопрос закрыт, и придержание
                    # зова снимается. Ход, которым проход открылся, ничего не
                    # закрывает: вопрос этого же хода оболочка читает запасным
                    # каналом раньше отметок, и снятое придержание позвало бы
                    # человека дважды.
                    if begun:
                        self.said_stuck = {}
                    begun, said = True, False
            ...
            if not self.head_alive():
                ...
                for word, _, _ in self.marks():
                    if word == TURN_STARTED:
                        begun = True
                    elif word in TURN_OVER and begun:
                        return word
                return ""
            quiet = time.time() - mute > self.secs(MUTE_ENV, MUTE_SECONDS)
            if not said and quiet:
                self.mute(n, int(time.time() - started))
                said = True
            if late and quiet:
                # Непарная отметка да получас тишины за нею. Пары у отметки нет
                # не потому, что ход был лишний, а потому, что «начат» пропал
                # вместе с журналом: журнал режут по размеру и сносят уборкой
                # дома. Ждать тут больше нечего, и проход кончается тем, что
                # есть. Человека про такую тишину зовёт mute.
                self.log("проход %d кончается непарной отметкой (%s): журнал молчит "
                         "%d с, парного «начат» не будет, %s"
                         % (n, late, int(time.time() - mute), self.id), 0)
                self.said_stuck = {}
                return late
            time.sleep(self.secs(WATCH_ENV, WATCH_STEP))

    def spare(self, n, word):
        """Отметка конца без парного «начат». Такую пишет вторая реплика головы
        на жалобу стоп-хука: ход у неё второй, а заказ был один. Проход она не
        кончает, но и молчать про неё нельзя. Снаружи непарная отметка
        неотличима от потерянного «начат», и разбирать потом придётся как раз по
        этой строке."""
        self.say("проход %d: отметка конца (%s) без парного «начат», проход идёт дальше"
                 % (n, word))
        self.log("непарная отметка конца хода (%s) на проходе %d у сессии %s: заказ был "
                 "один, проход не кончается, %s" % (word, n, self.sess, self.id), 0)
    ...
    def run_live(self):
        ...
        while True:
            ...
            if alive:
                ...
                mine = False
                continue
            # То же самое с фоновой командой оболочки. Голова запускает её и
            # уходит ждать конца. Ход при этом короток, строка стоит, а
            # воронка снимала окно вместе с командой (так встала голова
            # DK-941). Конец команды поднимает ход головы тем же порядком,
            # каким его поднимает конец субагента.
            running = self.shells()
            if running:
                self.log("проход %d кончился при живой фоновой команде (%s), %s: заказ не "
                         "подаётся, ход поднимет её конец"
                         % (n, ", ".join(running), self.id), 0)
                self.say("проход %d кончился при живой фоновой команде (%s): заказ не "
                         "подаётся, ход поднимет её конец" % (n, ", ".join(running)))
                sect = after
                mine = False
                continue
            ...
            if waited == WAIT_TAKEN:
                ...
                continue
            if waited:
                # Счёт холостых начинается заново: ожидание значит, что работа
                # шла, а воронка ловит подряд идущие пустые проходы. Без сброса
                # единица от утреннего прохода доживала до вечера и снимала
                # окно вместе с живой работой (DK-966).
                holds, idle = holds + 1, 0
    ...
    def run_passes(self):
        ...
        while True:
            ...
            # нужно тем же, чем живой голове: проход этот не холостой, потолка
            # он не тратит, и счёт холостых после него начинается заново
            # (DK-966).
