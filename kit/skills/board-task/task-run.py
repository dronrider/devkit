...
обоими каналами, и второй зов о нём придержан.
...
# Тишина между двумя зовами об одном и том же вопросе. Один запрос разрешения
# приходит обоими каналами, отметкой хода и строкой уведомителя, и без этого
# срока человек получал бы на него два баннера подряд.
STUCK_QUIET = 120
...
class Pipeline:
    def __init__(self, opts):
        ...
        self.note_seen = {}
        self.pending = []
        # Когда и на чём оболочка звала человека последний раз. Один вопрос
        # приходит двумя каналами, и второй зов о нём человеку не нужен.
        self.said_stuck = {}
    ...
    def notes(self):
        ...
        return [self.fields(line, NOTE_KEYS).get("повод", "-") for line in lines]
        ...
        while True:
            ...
            for i, (word, why) in enumerate(got):
                ...
                if word == TURN_WAITS and why in STANDING:
                    self.stuck(n, why)
            for why in self.notes():
                # Запасной канал. Отметки хода у сессии может не быть вовсе, а
                # вопрос она задала. Значит, человека зовём по строке
                # уведомителя. Тишину журнала это не отменяет, её называет mute.
                if why in STANDING:
                    self.stuck(n, why)
            if not self.head_alive():
    ...
    def stuck(self, n, why):
        ...
        уведомителя, и повторный зов об одном и том же вопросе придержан."""
        now = time.time()
        if now - self.said_stuck.get(why, 0) < STUCK_QUIET:
            return
        self.said_stuck[why] = now
