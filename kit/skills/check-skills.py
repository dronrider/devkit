...
def check_proofread(here):
    ...
    # DK-620: тринадцать пунктов правят фразу, а снесённую разметку и
    # подменённое утверждение ловит только инвариант и проход страницы.
    # Оба случая DK-606 прошли мимо пунктов. Три буллета README стали двумя
    # абзацами, а находка сторожа сменила утверждение про парсер.
    if "\n## Проход страницы" not in "\n" + sk:
        fails.append("proofread: нет прохода страницы, счёт пунктов и число блоков никто не сверяет")
    guard = section(sk, "## Что не трогает")
    if "утверждение" not in guard:
        fails.append("proofread: в «Что не трогает» нет инварианта правки готового текста")
    if "## Инвариант" not in pairs:
        fails.append("proofread: в pairs.md нет пары на инвариант разметки и утверждения")
    return fails
...
def check_prose(root):
    ...
    # Горячий список из девяти примет держится там же: пять дал замер DK-446,
    # четыре разбор 2026-09-12 (DK-962). Список короче это потерянная примета,
    # и заметить пропажу можно только по числам следующего замера, то есть
    # через цикл.
    fails = []
