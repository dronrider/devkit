...
class Loop:
    ...
    def turn_cmd(self, sid):
        ...
        некому. Имя витка («<ID> цель», DK-879) ставится флагом --name
        безусловно: каждый виток это свежий sid, resume тут не бывает, и
        клиент пишет то же имя записью custom-title в свой транскрипт, а
        дашборд читает её той же лестницей заголовка, что и у остальных
        известных голов."""
        turn = ["-p", "--session-id", sid, "--name", "%s цель" % self.id, self.prompt]
