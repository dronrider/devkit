...
каждого прогона свой HOME и свой git-репозиторий (obeycheck/README.md).

  obeycheck [флаги] <раскладка1> <раскладка2>

  --scenarios dir  директория сценариев (по умолчанию obeycheck/scenarios из
                   чекаута devkit)
  ...
  --home-seed dir  дерево, которое кладётся во временный HOME до раскладки:
                   там живёт машинная обвязка вроде авторизации харнеса
...
func main() {
	...
	homeSeed := fs.String("home-seed", "", "что положить во временный HOME до раскладки")
	...
	dir := *scenarios
	if dir == "" {
		dir = filepath.Join(root, "obeycheck", "scenarios")
	}
