...
// моделью (pickTier), а подъёмы дешёвых ярусов бакетов не добавляют, mini,
// base и pro тратят из одного week_all. budget читает потолок по тому, что
// корректор сделал с opus, вместо того чтобы прикидывать темп расхода сам:
// своё предсказание разошлось бы с корректором ровно там, где ошибка дороже
// всего, на подъёме opus -> fable.
