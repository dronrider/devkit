...
// goalListItemRe ловит начало пункта списка: маркер «- » (кандидат нарезки,
// DK-161, DK-490, DK-138) либо «N. » (нумерованный список, DK-156). Группа 1
// это остаток строки после маркера.
var goalListItemRe = regexp.MustCompile(`^(?:-|\d+\.)\s+(.*)$`)

// goalItemIDRe ловит ID в голове уже найденного пункта списка: сразу, форма
// без номера кандидата, которой материализует не только goal-cut, но и
// живые цели («DK-491 (task, M, R=61)...», «DK-149 (LLD): ...», «DK-153,
// ...»), либо после «кандидат N, », форма DK-161 (номер кандидата заменяется
// на ID при материализации, kit/skills/goal-cut/SKILL.md, «Список
// кандидатов»). У кандидата пробной нарезки, ещё не заведённого строкой
// доски («- кандидат N (...)»), своего ID нет ни в той, ни в другой форме, и
// такой пункт состав не пополняет.
var goalItemIDRe = regexp.MustCompile(`^(?:кандидат\s+\d+,\s*)?([A-Z][A-Z0-9]*-\d+)\b`)

// goalMaterializedRe ловит перечисление материализации во вводном абзаце
// раздела, до первого пункта списка: «DK-641 (кандидат 1), DK-642 (кандидат
// 2), ...» (тот же скилл). Признак состава это ID сразу перед «(кандидат N)».
// После первого списка раздела регексп не зовётся: за списком идут решения и
// сверки, а не состав, и там та же форма «ID (проза)» держит уже не
// материализацию, а разбор: пример живьём DK-156, «DK-117 (наблюдаемость
// витков из чата) в состав цели не вошла».
...
// goalTaskIDs собирает состав цели из раздела «Задачи цели»: вводное
// перечисление материализации перед списком и пункты первого списка раздела
// (кандидаты нарезки), не вся проза раздела. Первый список кончается первым
// абзацем не-пунктом после него: решения, сверки и границы, идущие следом
// («Порядок:», «Решения нарезки:» и подобные), в тот же список у файла не
// уложены, и tally их не читает вовсе, даже когда там мелькает форма
// «ID (проза)» без слова «кандидат». Порядок сохраняется, повторы
// отбрасываются, а ID самой цели пропускается: раздел ссылается и на неё.
func goalTaskIDs(lines []string, self string) []string {
	...
	inList, pendingBlank, done := false, false, false
	for _, raw := range lines {
		...
		m := goalListItemRe.FindStringSubmatch(t)
		if m == nil {
			if !inList {
				// Абзац до первого пункта списка: тут может лежать
				// перечисление материализации.
				for _, mm := range goalMaterializedRe.FindAllStringSubmatch(t, -1) {
					add(mm[1])
				}
				continue
			}
			if pendingBlank {
				// Список кончился пустой строкой и обычным абзацем: дальше
				// решения и сверки раздела, не состав.
				done = true
			}
			// Иначе это строка-продолжение обёрнутого пункта.
			continue
		}
		inList, pendingBlank = true, false
		if id := goalItemIDRe.FindStringSubmatch(m[1]); id != nil {
			add(id[1])
		}
	}
	return out
