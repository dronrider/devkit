...
const usageText = `agentctl: выбор исполнителя под задачу по метаданным доски (RULES.board.md)
  ...
  fit --goal <файл>       сверка бюджета цели с остатком бакетов до сброса на
                          постановке: первая строка машинная (fit: ok, tight
                          либо unknown), вторая называет бюджет, остаток и дату
                          сброса по каждому бакету раздела «Бюджет». Бюджет,
                          съедающий не меньше четырёх пятых остатка, даёт
                          tight; отказом это не становится, числа правит человек
  ...
  plan set <пункт>...     план работ сессии файлом ~/.devkit/plans: set кладёт
  plan step [<пункт>]     план целиком (пункт с переносами строк это список),
  plan done [<пункт>]     step начинает пункт и закрывает прежний идущий, done
  ...
  wait <ID> <условие>     отметка машинного ожидания для оболочки конвейера:
       ...
       [--check]          идёт. Условия: срок (ждём только время), есть <путь>,
                          нет <путь>, чисто <путь> (в дереве нет
                          незакоммиченного), слита <ID> (работа задачи в main),
                          закрыта <ID> (строка в архиве), процесс <pid> (конец
                          процесса), час <02:00 | 2026-09-12 02:00> (срок это
                          сам час, --until не нужен). Срок вида 90s, 10m, 1h,
                          потолок из [head] wait_cap профиля харнеса, по
                          умолчанию два часа: ожидание длиннее паркует строку
                          доски, а не оболочку. Событие, которое уже пришло,
                          даёт отказ. Отметка ложится файлом на задачу в
                          ~/.devkit/waits, оболочка вернётся к работе по событию
                          либо по сроку. --check отметки не кладёт и проверяет
                          условие сейчас: выход 0 пришло, 1 нет, 2 ошибка
...
func main() {
	...
	case "fit":
		...
		dir := fs.String("C", gdir, "стартовая директория")
		goal := fs.String("goal", "", "файл цели с разделом «Бюджет»")
		pos := frame.ParseArgs(fs, args[1:])
		needArgs(pos, 0, 0, "fit --goal <файл цели>")
		if *goal == "" {
			fail(fmt.Errorf("жду: fit --goal <файл цели>"))
		}
		root, rerr := findRoot(*dir)
		if rerr != nil {
			fail(rerr)
		}
		// Сверка идёт на постановке, до запуска цикла, поэтому в реестре
		// сторожка цель она не отмечает: заведённая цель не работает.
		msg, err = cmdFit(root, *goal, timeNow())
	case "lap":
	...
	case "exec":
		...
		os.Exit(code)
	case "plan":
		// Корень с доской команде не нужен: план лежит на уровне машины, как и
	...
	case "wait":
		...
		check := fs.Bool("check", false, "не класть отметку, а проверить условие сейчас: выход 0 пришло, 1 нет, 2 проверить нечем")
		pos := frame.ParseArgs(fs, args[1:])
		usage := "wait <ID> <условие> [<цель>] [--until <срок>] [--note <текст>] [--check]"
		needArgs(pos, 2, 4, usage)
		// Четвёртое слово бывает только у часа: дату и время человек пишет
		// через пробел и без кавычек.
		if len(pos) == 4 && pos[1] != waitHour {
			fail(fmt.Errorf("лишний аргумент %q: %s", pos[3], usage))
		}
		...
		if *check {
			...
			said, came, cerr := cmdWaitCheck(root, pos[0], pos[1], target, timeNow())
			if cerr != nil {
				logRun(2)
				fmt.Fprintln(os.Stderr, "ошибка:", cerr)
				os.Exit(2)
			}
			code := 1
			if came {
