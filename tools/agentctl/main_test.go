...
`)
	...
	// DK-908: свежесть проверяется фактом (нет слова «протух»), не цифрой
	// возраста. `run("quota")` сам не бесплатен: подпроцесс меряет возраст
	// собственным time.Now() в неизвестный момент после старта, и под нагрузкой
	// параллельного прогона (девять компонентов разом, находка при слиянии
	// DK-860) этот момент отодвигается на минуту и больше. Снимок,
	// записанный как пятиминутный, к проверке оказывался «возраст 6м» при том
	// же самом факте свежести, и жёсткое сравнение со строкой «возраст 5м»
	// красило зелёный код. Число минут ниже подтверждает только то, что
	// возраст вообще посчитан и не мог получиться меньше заложенных пяти,
	// а не конкретную стенную секунду.
	out, err := run("quota")
	if err != nil || strings.Contains(out, "протух") {
		t.Fatalf("пятиминутный снимок без настройки должен быть свежим (%v):\n%s", err, out)
	}
	if age := quotaAgeMinutes(t, out); age < 5 {
		t.Fatalf("возраст снимка меньше заложенных пяти минут: %dм\n%s", age, out)
	}
...
// quotaAgeMinutes достаёт число минут из строки «возраст Nм» в выводе quota.
// Само число зависит от стенного времени подпроцесса и заведомо плавает под
// нагрузкой (DK-908), поэтому вызывающий тест сверяет его не с точной
// величиной, а с фактом (протух или нет) и, самое большее, с нижней границей,
// которую подмять стенным временем нельзя.
func quotaAgeMinutes(t *testing.T, out string) int {
	t.Helper()
	m := regexp.MustCompile(`возраст (\d+)м`).FindStringSubmatch(out)
	if m == nil {
		t.Fatalf("в выводе нет возраста снимка:\n%s", out)
	}
	n, err := strconv.Atoi(m[1])
	if err != nil {
		t.Fatalf("возраст снимка не число %q: %v", m[1], err)
	}
	return n
}
