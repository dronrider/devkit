...
// reviewShift переводит исполнительский вердикт в ревьюверский. Ревьювер идёт
// тем же ярусом, каким писался дифф: пилот DK-1159 показал, что на спуске
// теряются серьёзные дефекты, а выигрыш по квоте не окупает пропуск. Пол
// base остаётся: mini дифф читает бегло и замечаний не находит, а base это
// дешёвая рабочая ступень. Два случая спуска не знают. Дизайн (тип LLD)
// читается тем же калибром, каким пишется: правило для него было отдельным
// ещё до снятия спуска и этой правкой не меняется. Грумминговый вердикт
// значит, что работы ещё не было, и
...
func reviewShift(v *verdict, r row) {
	...
	case tierMini:
		...
		v.Tier = tierBase
	default:
		v.Reason += fmt.Sprintf("; роль ревью: %s остаётся, ревьювер идёт ярусом исполнителя, спуска на ступень нет", v.Tier)
	}
}
...
func pickVerdict(root, id, role, goal string) (pickResult, error) {
	...
	// Роль ревью применяется последней по ярусной оси: пол base считается от
	// того, что осталось после override и корректора, иначе корректор увёл бы
	// вердикт ревьювера ещё на ярус ниже пола.
	if role == roleReview {
		reviewShift(&v, *r)
	}
