...
type planAddr struct {
	...
	label string
	file  string
	tmux  bool // sid взят из DEVKIT_TMUX, а не из ID сессии
}
...
// в окружении нет (DK-852), и чужой план узнаётся по тексту: одного совпавшего
// пункта хватает, чтобы набор считался перекладкой своего плана. Изъятие одно,
// и оно у адреса из DEVKIT_TMUX: имя tmux-сессии переживает сессию, новая
// сессия под тем же именем (DK-269) кладёт свой план на место доделанного, и
// терять там нечего. У адреса из ID сессии доделанный план бережётся так же:
// диспетчер, закрывший все пункты, иначе терял бы запись о сделанном под
// безымянным планом субагента.
