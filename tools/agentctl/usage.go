...
//
// Случай «панель ни разу не отозвалась знакомым словом» не сводится к одной
// причине: за тем же кадром прячется и смена разметки, и отрисовка, не
// успевшая за отведённый срок, и чужой экран, перехвативший ввод, и Enter,
// не долетевший до клиента. Разбор 27.08 (DK-457) это как раз показал: три
// захода реконструкции ушли на то, что отказ назвал одну версию из четырёх
// как установленную. Сообщение поэтому перечисляет версии, а разбор по кадру
// уже выбирает между ними.
func panelFailure(q *quotaSpec, pane string, why error) error {
	...
	case !panelSeen(pane):
		fmt.Fprintf(&b, "панель %s не подала ни одного знакомого слова за %s. Причина не установлена, версий несколько: разметка панели могла смениться, отрисовка не успела уложиться в срок, экран перехватило что-то ещё, либо Enter не долетел до клиента.", usageCommand, usagePanelTimeout)
	case why == nil:
