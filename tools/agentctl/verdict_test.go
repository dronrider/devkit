...
// машинные строки tier и via снимаются, слова ярусов в человеческой строке
// разворачиваются по таблице псевдонимов. Иначе сверять новый вывод было бы не с чем: снимок с
...
func asModelVerdicts(s string) string {
	...
	for _, ln := range strings.Split(s, "\n") {
		if strings.HasPrefix(ln, "tier: ") || strings.HasPrefix(ln, "via: ") {
			continue
		}
