...
// позиции, как -h у соседней helpRequested: команда стоит после «--», и версия
// не должна зависеть от того, что написано рядом. Формат строки один на все
...
func printVersion(args []string, w io.Writer) bool {
	for _, a := range args {
		...
		// полезная нагрузка. У cmdout там лежит команда прогона, и --version
		// внутри неё принадлежит ей; напечатать вместо прогона версию и выйти
		// нулём значит соврать обвязке, что прогон прошёл.
		if a == "--" {
			break
		}
