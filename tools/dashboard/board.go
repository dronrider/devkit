...
func (s *server) liveWorks(projectPath, prefix string, board json.RawMessage) []Work {
	...
	// а хранимого состояния по нему нет (DK-904). Машину без tmux называет
	// tmuxMissingCheck на уровне ответа, и журнал ею не засоряется.
	var sessions []tmuxSession
	if tmuxMissingCheck() == "" {
		var err error
		if sessions, err = tmuxList(); err != nil {
