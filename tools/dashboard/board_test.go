...
func TestStaticBoardRowActions(t *testing.T) {
	...
	// Кнопок в колонке две, работа и разговор, а выбор подписки с уровнем
	// открывается всплывашкой самой кнопки запуска. Трёх точек тут больше нет:
	// под ними лежал ровно этот выбор, и стояли они у одной строки, а у соседней
	// нет (замечание пользователя).
	for _, want := range []string{`el("span", "racts")`, `main.addEventListener("contextmenu"`,
	...
	if strings.Contains(body, "rdots") {
		t.Error("три точки вернулись в строку: состав кнопок снова зависит от того, " +
			"нашлось ли что положить в меню")
	}
...
// Рубеж бокового отступа ячейки таблицы: меньше подпись читается приклеенной к
// границе колонки. Число тут не из головы, оно стоит в static/style.css одним
// правилом на th и td, и стенд сверяет замер с ним.
const tblSideMin = 12
