...
// (блок вопроса напечатан), и класть в файл, который никто не прочитает,
...
func (s *server) handleTaskMessagePost(w http.ResponseWriter, r *http.Request) {
	...
	case parkedByAsk(row):
		...
		resp["message"] = fmt.Sprintf(
			"реплика легла в чат %s основного чекаута: %s. Строка ждёт с лежащим ответом, подъём повторит тик сторожка",
			name, rep.Line)
	}
