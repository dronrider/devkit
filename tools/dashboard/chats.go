...
type chatEntry struct {
	...
	AITitle string `json:"-"`
	// TitleNamed говорит, что заголовок поставлен дашбордом или человеком
	// (custom-title либо известное имя витка цели), а не собран эвристикой
	// из текста первой реплики. Только тогда ведущий номер в заголовке
	// безопасно гасит чип с тем же ID (chatTitleTask в static/app.js):
	// иначе свободный чат со случайно совпавшим ai-title терял бы чип
	// задачи по текстовому совпадению, а не по факту привязки (замечание
	// ревью DK-879).
	TitleNamed bool `json:"titleNamed,omitempty"`
	...
	// Quota называет словами разговор, вставший на ретрае кончившейся
	// подписки. Харнес не пишет отказа никуда, сокет отвечает как у живого, и
	// без разбора экрана такой разговор неотличим от занятого агента (материал
	// разбора DK-647, 480 сессий за 10 дней). Причина и срок сброса едут одной
	// фразой. Списку и ответу на реплику нужна одна и та же строка. Пусто у
	// разговора без такого следа на экране.
	...
	// Past это прошлые заходы разговора, старшими первыми: сессии, кончившиеся
	// под тем же именем окна. Строкой списка они больше не стоят, их лента и
	// их задачи приехали сюда, а панель называет их числом заходов.
	Past []chatPass `json:"past,omitempty"`
	// win это ключ склейки: проект и имя окна, под которым сессия поднималась.
	// Наружу не едет, служит одному обходу списка.
	win string
...
func (s *server) chatEntriesFrom(files []chatFile, limit int, win chatWindow) ([]chatEntry, bool) {
	...
	for _, f := range files {
		...
		// (DK-859).
		// Ключ склейки идёт по тому же имени, что и строка: у разговора с
		// вырезанной записью рождения имя из журнала пустое, и заходы одного
		// окна расходились бы по строкам (DK-430.15).
		e.win = chatLife(chatWin(f.projPath, winName), last.Time, winSinceOf(winName))
		...
		// Кончившаяся подписка. Харнес не пишет отказа никуда и продолжает
		// отвечать сокетом как живой, единственный след это его собственный
		// ретрай в пейне (материал разбора DK-647). Мера своя, а не род Stuck.
		// Клин лечится перезапуском, кончившуюся подписку перезапуск не
		// приближает ни на минуту, снятый процесс встанет на тот же ретрай
		// заново.
		if e.Tmux != "" && alive(e.Tmux) {
			if retry, weekly := s.quotaWaitOf(e.Tmux); retry {
				harness := harnessOfRoot(view, s.cfg.Home, f.root)
				e.Quota = quotaWaitNote(s.quotaResetOf(harness, weekly))
			}
		}
...
func chatCmd(env, model, resume, text, name string, h *Harness, agentctl string) string {
	...
	cmd := head + client
	// Ярус называется явно всякой подписке: клиент второй подписки принимает
	// имя из своей лестницы, и сессия поднимается выбранной моделью, а не
	// дефолтом профиля (проба DK-750: заказ под обвязкой agentctl exec отвечает
	// и выходит нулём). Прежний заказ без флага держался на предупреждении
	// unrecognized_model из панели DK-269, а оно оказалось жалобой клиента на
	// незнание окна контекста модели и оставлено осознанно (файл задачи).
...
// headlessEnv это окружение сессии печатного режима: общие пары и за ними метка
// подъёмщика. Дорога такая одна, конвейер задачи, и метка едет только ей.
// Разговор, разбор черновика и окно входа поднимают живой REPL: там фон законен,
...
//
// Своя метка вычищается наравне с чужими переменными, и по той же причине.
// Наследуется она тем же каналом: сессия конвейера метку несёт, а всё, что из
// неё заведено, донесёт её до себя. Снимает её тут подъёмщик, и потому живое
// окно, поднятое с экрана, метку получить не может ниоткуда, кроме как от
// headlessEnv.
...
// launchEnv это пары окружения поднятой сессии, одни на все дороги подъёма:
// разговор, конвейер задачи, цикл цели, разбор черновика и окно входа. Своего
...
// конвейер с разбором звали её сбоку. Метки печатного режима тут нет, её ставит
...
// chatSendCheck это пауза перед сверкой живости. Полоса молчания шире одной
// подачи. Клиент, вышедший уже после Enter, хода тоже не начинает, а имя его
// сессии на машине ещё стоит, и сверка вплотную к посылке читает такую подачу
// удачей (замечание ревью DK-863). Пауза эту полосу и ловит.
//
// Срок взят тот же, что у chatSendPause. Та выбрана по отрисовке поля ввода, а
// ход начинается сразу за отрисовкой. Ради чего паузу предлагали в замечании,
// хватило бы и меньшего: замер снял с сервера tmux от 6 до 12 мс на снятие
// вышедшей сессии, а между смертью клиента и сверкой стоит целая посылка
// Enter, это ещё пять миллисекунд. Платит за паузу ответ ручки `/say`, он
// растёт на четверть секунды.
var chatSendCheck = 250 * time.Millisecond
...
var (
	...
	// и непошедший браузер. Стадия ввода кода в набор не входит: подсказки
	// навигации у неё нет вовсе, и читается она простоем. Клавиш туда не идёт
	// и так, простой их не шлёт.
...
// paneWindow вырезает из снимка открытое окно клиента и говорит, открыто ли
// оно вообще. Ищутся слова окна только в этом куске, и это единственная защита
// от эха. Слова меню отката лежат в исходниках самого дашборда и в тексте
// задачи, то есть их печатает в свою ленту всякий агент, который тут работает.
// Ищи их по всему снимку, и окно с идущим ходом читалось бы меню отката
// (замечание ревью 15).
//
// Открытое окно выдаёт место подсказки навигации. Пока окно держит клавиатуру,
// клиент убирает и поле ввода, и свою строку режима, и ниже подсказки на
// экране не остаётся ничего. Сверху окно отрезается по той же строке режима,
// если она на экране осталась.
func paneWindow(text string) (string, bool) {
	...
	top := 0
	for i := hint; i >= 0; i-- {
		if paneModeRe.MatchString(lines[i]) {
			top = i + 1
			break
		}
	}
...
// paneState читает снимок. Первым спрашивается открытое окно клиента. Нет его,
// значит клавиатура у человека, и остаётся выбор между идущим ходом и
// простоем. Есть, значит окно узнаётся своими словами. Меню отката зовут
// заголовок и строка про восстановление кода и разговора, экран входа зовут
// слова его стадий, а виджету вопроса своих слов не нужно: он и есть всякое
// другое открытое окно. Слова ищутся только внутри окна, потому что снаружи
// они стоят в ленте разговора чужим текстом.
func paneState(text string, ok bool) string {
	if !ok || strings.TrimSpace(text) == "" {
		return paneBlind
	}
	win, open := paneWindow(text)
	if !open {
		if strings.Contains(strings.ToLower(text), paneTurnMark) {
			return paneTurn
		}
		return paneIdle
	}
	low := strings.ToLower(win)
	if paneHasAny(low, paneRewindMarks) || paneHeadIs(win, paneRewindHead) {
		return paneRewind
	}
	if paneHasAny(low, paneLoginMarks) || paneHeadIs(win, paneLoginHead) {
		return paneLogin
	}
	return paneAsk
}
...
// Второй источник тут журнал самой сессии.
//
// Доводов нужно два, и вместе они значат «агент сейчас в ходе». Первый это
// незакрытый вызов инструмента в хвосте. Второй это движение: журнал писался
// только что. Одного вызова мало, признак этот нарочно широкий, им держится
// занятость сессии полчаса. Вызов, брошенный прерванным ходом или несданной
// работой делегата, висит в хвосте сколько угодно, и по нему сомнение звучало
// бы над спокойным окном, где всё в порядке (замечание ревью 16). Одной
// свежести тоже мало, журнал остаётся свежим ещё двадцать секунд после конца
// хода, и сомнение звучало бы после каждого удачного стопа.
//
// Цена такой пары в том, что агент, ушедший в долгий инструмент, журнала не
// трогает минутами, и смена подсказки на нём останется незамеченной до
// следующего нажатия. Ложное обвинение клиента стоит дороже пропущенного
// случая: первое человек читает как поломку, второе оставляет прежнее
// поведение стопа.
func (s *server) chatIdleDoubt(projPath, sid, name string) string {
	...
	if e.open <= 0 || s.now().Sub(e.last) >= busyFresh {
		return ""
	}
	s.logf("стоп чата %s: подсказки клиента не узнал, окно %s кончается словами %q, "+
		"а журнал сессии пишется прямо сейчас и держит незакрытый вызов", sid, name, paneLastWords(name))
	return "подсказки клиента не узнал: в окне нет ни знакомого признака хода, ни знакомого " +
		"экрана, а журнал сессии пишется прямо сейчас и держит незакрытый вызов; клавиш туда не послано"
}
...
// список чатов, стоп под перезапуск, прерывание хода и снятие дожима зовут её
// же (DK-793). Прежде ручки читали одну запись журнала, а список рядом
// добавлял к ней слово живого клиента, и тот же чат список видел в окне, а
// действие над ним окна не находило: запись рождения вырезается ротацией
// журнала, и разговор старше пары дней делался для ручек чужим.
//
...
// Последней память окна (mem, её собирает chatWinMemory): окно, поднятое
// дашбордом резюмом этого разговора, помнит его как родителя, и до записи
// хука старта другого хозяина у имени нет. Из нескольких таких окон берётся
// поднятое позже всех, окно без отметки подъёма мертво и не считается. Пустое
// имя значит, что ни один источник окна не назвал, а вторым значением идёт
// источник, у которого имя нашлось.
func (s *server) chatWinOf(sid string, recs map[string][]sessionBind, live map[string]peer,
	mem map[string]chatStore) (string, string) {
	if p, ok := live[sid]; ok {
		if n := peerTmux(p); n != "" {
	...
	best, at := "", int64(0)
	for name, st := range mem {
		if st.From != sid || st.Raised <= 0 || st.Raised < at {
			continue
		}
		if sessions.TmuxOwner(recs, name) != "" || tmuxHeld(live, name) != "" {
			continue
		}
		best, at = name, st.Raised
	}
	if best != "" {
		return best, chatWinByMemory
	}
	return "", ""
}

// chatWinMemory читает память окон под присмотром по разу на имя: имён
// горстка, а зовущих свёртку строк списка десятки, и читать файл на каждую
// строку дорого (замечание ревью DK-793).
func (s *server) chatWinMemory() map[string]chatStore {
	mem := map[string]chatStore{}
	for _, name := range s.chatWatchNames() {
		mem[name] = s.chatStoreRead("tmux-" + name)
	}
	return mem
}
...
// рубеж живости, что у состояния vscode в списке чатов.
func (s *server) chatForeignLive(projPath, sid string) bool {
	info, ok := findSession(s.transcriptRoots(), projPath, sid)
	return ok && info.mod.After(s.now().Add(-sessionLiveTTL))
}
...
func (s *server) handleChatStop(w http.ResponseWriter, r *http.Request) {
	...
	last.Tmux, _ = s.chatWinOf(sid, recs, s.peers(), s.chatWinMemory())
	alive := tmuxAliveFn()
	if body.Drop {
		...
		if last.Tmux == "" {
			...
			writeJSON(w, http.StatusOK, map[string]any{"way": "gone", "tmux": "",
				"message": "снимать нечего: разговора в нашей tmux не было, поднимется резюмом"})
			return
		}
...
func (s *server) handleChatSay(w http.ResponseWriter, r *http.Request) {
	...
	if term != "" && !stuckDialog {
		...
		// Ретрай кончившейся подписки. Харнес взял реплику клавишами так же
		// охотно, как живой (терминал цел), а ходу она не дала и не даст до
		// сброса окна. Молчание тут прежде и было единственным исходом
		// (материал разбора DK-647). Статус едет и в ответ ручки, и в ленту
		// пометкой. Человек узнаёт причину сразу, а не по часу тишины.
		if retry, weekly := s.quotaWaitOf(term); retry {
			harness := harnessOfRoot(s.harnesses(), s.cfg.Home, info.root)
			note := quotaWaitNote(s.quotaResetOf(harness, weekly))
			out["stuck"] = note
			s.saidMark(saidSessionKey(sid), note)
			s.logf("реплика чата %s легла в очередь квоты: %s", sid, note)
		} else if why := s.chatStuck(sid); why != "" {
			out["stuck"] = why
...
// пузырь с этой причиной строкой.
...
// chatStuckMark отдаёт то же суждение и вдобавок отметку записи, на которой оно
// стоит. Пока отметка не сменилась, в журнале уведомителя про эту сессию ничего
// нового не произошло. Срока у липкости нет вовсе, и мерить её временем нечем
// (замечание ревью DK-695: живой случай 01.09 держал липкость 5м50с, то есть
// дольше окна askQuietWindow).
//
// Отметка это счёт повторов строки плюс сама строка. Одного текста мало: время
// hooks/notify.py пишет до секунды, своего номера у записи нет, и два вопроса
// разрешения в одну секунду дают две дословно одинаковых строки. Тогда новый
// вопрос прятался за старым ответом, и панель молчала ровно там, где молчать
// перестала (второе замечание ревью). Счёт повторов взят у того же приёма в
// kit/skills/board-task/task-run.py (fresh), где секундная точность харнеса
// ломает память по строке так же.
func (s *server) chatStuckMark(sid string) (why, mark string) {
	...
	for i := len(lines) - 1; i >= 0; i-- {
		...
		mark = notifyMark(lines[:i+1], lines[i])
...
// notifyMark метит запись журнала счётом её повторов до этого места
// включительно. Позиции в файле тут нет нарочно, и это тот же довод, что у
// fresh в kit/skills/board-task/task-run.py: журнал уведомителя режется по
// размеру, и после реза смещение прошлого чтения указывает в середину чужой
// строки, а нумерация строк уезжает молча и на любое число. Усушка сбивает и
// счёт, но сбивает его в безопасную сторону: отметка расходится, ответ панели
// перестаёт объяснять пустой снимок, и строка журнала снова зовёт чинить
// разбор. Спрятанный вопрос обходится дороже лишней жалобы.
func notifyMark(upto []string, line string) string {
	seen := 0
	for _, ln := range upto {
		if ln == line {
			seen++
	...
	return fmt.Sprintf("%d|%s", seen, line)
}
...
func (s *server) handleChatAsk(w http.ResponseWriter, r *http.Request) {
	...
	name, _ := s.chatWinOf(sid, s.bindsAll(), s.peers(), s.chatWinMemory())
	if name == "" {
...
// нечего (замечание пользователя про DK-460). touchCmds в taskctl держит тот
// же список: команды груминга (add, set, file, dep, progress) в него не
// входят (DK-1121).
...
func (s *server) titleFill(list []chatEntry) {
	for i := range list {
		...
		known := ""
		if e.Goal != "" {
			// Виток цели ставит себе --name сам (goal-run.py, DK-879) и
			// приезжает уже с custom-title из транскрипта. Запасной путь тут
			// на случай старого транскрипта, поднятого до этой правки, либо
			// клиента без флага --name: имя считается той же лестницей, что
			// у остальных известных голов, ID и роль словом.
			known = e.Goal + " цель"
		}
		e.Title = titleFor(e.Summary, e.CustomTitle, known, e.AITitle, e.Title)
		e.TitleNamed = e.CustomTitle != "" || known != ""
	}
}
