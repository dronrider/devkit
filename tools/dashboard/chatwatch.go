...
func (s *server) chatRaised(sess, sid, task, proj string) {
	...
	// Момент подъёма помнится и вторым полем, которое не стирают ни смерть
	// окна, ни снятие присмотра: по нему заходы нового жильца имени отделяются
	// от разговоров прежнего (DK-859).
	st.Since = st.Raised
	...
	// первый же ход.
	stopWaitClear(&st)
	// Родитель резюма стирается по той же причине: имя достаётся новому
	// жильцу, а память окна с прежним From отдавала бы старому разговору
	// чужое окно, пока новый не записался ни в журнал, ни в реестр клиента
	// (замечание ревью DK-793).
	st.From = ""
