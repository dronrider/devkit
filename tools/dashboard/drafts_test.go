...
func TestStaticBoardTableHead(t *testing.T) {
	...
	// работы только у сессий: приём один, состав колонок свой. Колонка хода
	// зовётся ключом, а не подписью: подписи у неё нет вовсе, слово в шапке
	// требовало под себя восемьдесят точек под кружок в девять.
