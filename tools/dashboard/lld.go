...
func taskLinks(projectPath, id, link, text string, rows map[string]boardRow, after, blocks []string) map[string]any {
	...
	// прочего), 1 это остальные открытые, 2 это закрытые. Ранг берётся со
	// строки доски; у закрытых и упоминаний без строки его нет, ноль честно
	// уводит их за ранжированных соседей.
