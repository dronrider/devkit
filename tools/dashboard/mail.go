...
// доставки, которые кладёт почтальон hooks/inbox.py, и правило живости витка,
// по которому пишется плашка чата. Своего состояния тут нет, читаются те же
// файлы, что пишут почтальон с оболочкой.

// mailStamp это формат времени в отметке и в записи реестра целей: тот же
// «%Y-%m-%dT%H:%M:%S» без пояса, каким его пишут hooks/inbox.py и
...
// почтальон движением цели не считает, и дашборд не считает тоже: иначе
// доставка держала бы цель живой сама за себя.
const mailSayWord = "почта"
...
// kit/skills/goal-loop/goal-run.py) и почтальон. Отказ по правам значит чужой
...
// значит не своя: строку доставки почтальон кладёт в тот же журнал.
...
// живёт почтальон: живой pid в замке цели, а без замка свежая метка движения
