...
// Почта цели со стороны дашборда: отметки доставки почтальона (hooks/inbox.py)
// и живость витка, по которой пишется плашка чата. Стенды кладут те же файлы,
// какие на живой машине кладут почтальон с оболочкой, настоящих сессий тут нет.

// mailBody собирает файл отметок тем же форматом, каким его пишет почтальон:
...
// поставил отметку, дашборду неважно: у почтальона и у ключа --ask ящик один,
...
func TestMessageDeliveredFromMarks(t *testing.T) {
	...
	if got.Delivered[0].Line != lying[0] || got.Delivered[0].Session != "8f2a1c30-1111" {
		t.Errorf("отметка почтальона потеряла строку или сессию: %+v", got.Delivered[0])
	}
...
// Живость витка меряется правилом почтальона, а не списком живых работ: замок
...
func TestGoalLiveByLockNotByWorks(t *testing.T) {
	...
	if got := getMessage(t, c, e, "XR-100"); !got.Live {
		t.Error("живой замок цели не дал живости: почта идущему витку доезжает")
	}
}
...
// движением не считается, иначе почтальон держал бы цель живой сам за себя.
func TestGoalLiveByMovedMarks(t *testing.T) {
	...
	watchEntry(t, e, "XR-100", stampNow(mailMoved-time.Minute))
	if got := getMessage(t, c, e, "XR-100"); !got.Live {
		t.Error("метка свежее порога не дала живости: порог разъехался с почтальоном")
	}
	...
	writeCycleLog(t, e, "XR-100", stampNow(5*time.Hour)+" виток 3: конвейер закрыт\n"+
		stampNow(time.Minute)+" почта: витку доставлена реплика «стой, не туда»\n")
	if got := getMessage(t, c, e, "XR-100"); got.Live {
		t.Error("строка доставки сошла за движение цели: почтальон держит цель живой сам за себя")
	}
...
// и Go, и разъехаться им нельзя: почтальон перестанет доставлять раньше, чем
// плашка перестанет обещать минуты. Сведёнными их держит этот тест.
func TestGoalLiveRuleMatchesPostman(t *testing.T) {
	path := filepath.Join("..", "..", "hooks", "inbox.py")
	data, err := os.ReadFile(path)
	if err != nil {
		t.Skipf("почтальона рядом нет: %v", err)
	}
	text := string(data)
	if want := fmt.Sprintf("MOVED = %d * 3600", int(mailMoved.Hours())); !strings.Contains(text, want) {
		t.Errorf("порог метки движения разъехался с почтальоном: в inbox.py нет %q", want)
	}
	if want := fmt.Sprintf("SAY_WORD = %q", mailSayWord); !strings.Contains(text, want) {
		t.Errorf("слово строки доставки разъехалось с почтальоном: в inbox.py нет %q", want)
	}
	if want := fmt.Sprintf("STAMP = %q", "%Y-%m-%dT%H:%M:%S"); !strings.Contains(text, want) {
		t.Errorf("формат времени отметки разъехался с почтальоном: в inbox.py нет %q", want)
	}
}
