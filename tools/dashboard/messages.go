...
func (s *server) handleGoalMessageGet(w http.ResponseWriter, r *http.Request) {
	...
	// человек»): отметки подхвата лежат в .devkit/goal-<ID>.mail, своей
	// ручки каналу не нужно. Отметку ставит и подхват, и ключ --ask
	// оболочки, и различать их клиенту незачем: состояние у строки одно.
	// Рядом идёт живость витка: поле осталось у ручки, хотя чат его больше не
	// рисует. Пустые «Входящие» отдаются пустым списком и без слов: пустая
	// коробка под лентой это просто пустой чат, а сбой чтения клиент рисует
	// своим узлом ошибки (DK-957).
