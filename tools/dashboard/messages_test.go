...
// витка». Пустой раздел отдаётся пустым списком и молча: служебные слова про
// непрочитанное сняты вместе с подсказками панели (DK-957).
func TestMessagePendingList(t *testing.T) {
	...
	text := body(t, resp)
	if resp.StatusCode != http.StatusOK {
		t.Errorf("пустые «Входящие» не прочитались: %d %s", resp.StatusCode, text)
	}
	if strings.Contains(text, "note") || strings.Contains(text, "пусто") {
		t.Errorf("ручка снова говорит словами о пустоте: %s", text)
	}
...
// Экран чата честен словами макета, и слов этих ровно столько, сколько нужно.
// Обещание доставки несёт сама реплика: лежащая подписана «ждёт витка», и по
// этой подписи видно, что цикл стоит. Плашки над полем ввода и служебной
// строки про пустые «Входящие» тут больше нет (DK-957): человек читал их на
// каждой цели заново и закрывал крестиком.
func TestStaticChatHonesty(t *testing.T) {
	...
	for _, gone := range []string{
		...
		if strings.Contains(text, gone) {
			t.Errorf("в static/app.js вернулась снятая подсказка %q", gone)
		}
	}
}
