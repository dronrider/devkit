...
// DK-934: отказ ворот ёмкости взведённой строке едет в ленту типом события про
// строку доски. Без повода в разборе лента положила бы его в прочие, и фильтр
// «задачи» такого события не нашёл бы.
func TestNotificationsArmIsTaskEvent(t *testing.T) {
	e := newTestEnv(t)
	writeNotifyLog(t, e.home, []string{
		"2026-09-12T01:40:00 сессия - повод task_arm уровень громкий бэкенд terminal-notifier цель - код возврата: 0 задача XR-213 проект devkit текст «devkit: XR-213 ждёт ёмкости» «потолок деревьев 5 из 5»",
	})
	out := getFeed(t, e, "?kind=task")
	if len(out.Items) != 1 {
		t.Fatalf("отказ ворот взвода не попал в фильтр задач: %d событий", len(out.Items))
	}
}
