...
func (s *server) saidSay(key, wire, way string) {
	...
	// Человек написал в разговор: стоп он передумал, и дожимать в нём ходы
	// сторожу больше нечего (stopwait.go). Дорог у реплики полдесятка, от
	// клавиш в своё окно до сокета клиента, и сходятся они здесь.
	s.stopWaitOffSaid(key)
}
