...
func TestRoundRaisesSession(t *testing.T) {
	...
	// DK-847: второй круг поднимает тик сторожка, а не человек, и список
	// чатов панели эту сессию не показывает.
	if !strings.Contains(got, hiddenEnv) {
		t.Errorf("второй круг поднят без признака hidden: %s", got)
	}
}
