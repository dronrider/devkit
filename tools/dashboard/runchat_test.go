...
func TestRunStopListsSeveralChatWorks(t *testing.T) {
	...
	appendBinds(t, e.home, fmt.Sprintf(
		"2026-08-10T09:59:40 сессия %s задача XR-004 проект demo дерево %s "+
			"транскрипт "+standTranscript(e.home, "t2")+" источник заказ повод startup tmux chat-XR-004-2\n"+
...
// Плашка работы это состояние чата, а не след нажатия (DK-893). Прежде её
// поднимало только нажатие «Отправить» в этой же панели, и ход, начатый до
// открытия чата, выглядел мёртвым разговором: шестиминутное размышление
// человек смотрел на пустую ленту. Второй дырой был потолок в десять минут,
// после которого плашка гасла при живом процессе. Стенд рисует панель чата в
// поддельном DOM (testdata/poc_chatbusylive.mjs) и двигает часы руками: ход
// длиннее потолка иначе не проиграть. Без node шаг пропускается: узел стенда,
// а не рабочей части.
func TestStaticChatBusyPlateLivesByStatus(t *testing.T) {
	node, err := exec.LookPath("node")
	if err != nil {
		t.Skip("node не найден: стенд плашки работы пропущен")
	}
	out, err := exec.Command(node, filepath.Join("testdata", "poc_chatbusylive.mjs"),
		filepath.Join("static", "app.js")).CombinedOutput()
	if err != nil {
		t.Fatalf("плашка работы по состоянию чата: %v\n%s", err, out)
	}
	t.Log(strings.TrimSpace(string(out)))
}
