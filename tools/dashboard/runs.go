...
func clientCommand(agentctl string, h *Harness, model string) string {
	if h == nil {
		// Режим разрешений ставится и своей подписке. Проход внутри оболочки
		// идёт headless (`claude -p`), окна у него нет, и запрос разрешения
		// одобрить некому: клиент отвечает отказом, а работа встаёт молча.
