...
func (s *server) sessionWorks(projPath, prefix string, rows map[string]boardRow,
	busy, alive map[string]bool) []Work {
	works := []Work{}
	...
	for _, f := range sessionFiles(s.transcriptRoots(), projPath) {
		// Список идёт свежими сверху, и остывший транскрипт обычно значит
		// оконченную работу. Обычно, но не всегда. Агент, отдавший работу
		// субагенту, своего транскрипта не трогает, пока тот не вернётся, и
		// держится это часами: живой случай DK-818, где транскрипт молчал
		// двадцать пять минут, а боковой журнал субагента писался минуту назад.
		// Такая сессия выпадала из списка работ целиком, и строка показывала
		// вчерашний труп конвейера вместо сегодняшней работы (четвёртая
		// приёмка, третий случай).
		//
		// Смотрим дальше только у сессий с живым окном: ходить в каталог
		// боковых журналов на каждый остывший транскрипт дорого, а работа без
		// живого окна и правда кончилась.
		if f.mod.Before(cutoff) {
			name := strings.SplitN(binds[f.ID].Tmux, ":", 2)[0]
			if name == "" || !alive[name] || !s.subBusyOf(f.path, now) {
				continue
			}
		}
...
// chat (DK-614), имени приложения в нём нет нарочно: по нему агент узнаёт
