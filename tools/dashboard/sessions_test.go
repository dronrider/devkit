...
// фишку, а разбор сессий живёт в разделе «Агенты». Над доской от ленты сессий
// осталась полоска с числом работ и дорогой в раздел: карточки повторяли его
// же, занимая строку над самой доской (решение пользователя).
func TestStaticInteractiveWork(t *testing.T) {
	text := readFile(t, filepath.Join("static", "app.js"))
	live := funcBody(t, text, "function renderLive(")
	for _, want := range []string{"работает ", `plural(n, "агент"`, `goKeepingChat("/agents")`} {
		if !strings.Contains(live, want) {
			t.Errorf("в полоске работ нет %q", want)
		}
	}
	// Ни кнопки стопа, ни карточек с разговорами: и то и другое живёт в
	// разделе «Агенты», а полоска только считает и уводит туда.
	...
	if strings.Contains(live, "openChat(") {
		t.Error("полоска работ снова открывает разговоры: сессии живут в «Агентах» и в чате")
	}
