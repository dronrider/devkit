...
func (s *smoke) stepTurn() (string, error) {
	...
	// Опустевшие «Входящие» отдаются без слов: служебную строку про
	// непрочитанное сняла DK-957, и вернуться она может только правкой ручки.
	if pending.Note != "" {
		return "", fmt.Errorf("ручка «Входящих» снова говорит словами: %q", pending.Note)
	}
	...
	return "сообщение подхвачено записью витка, «Входящие» опустели и слов о том не пишут", nil
}
