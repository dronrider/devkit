...
func TestBoardRunsCarriesStage(t *testing.T) {
	...
	// Сессий этой машины у строки нет вовсе, и признака у неё нет тоже:
	// прежний other («исполнителя не видно») снят.
	gone := rows["XR-002"]
	if gone.Run != "" {
		t.Fatalf("строка без сессии помечена %q, жду пустой признак", gone.Run)
	}
