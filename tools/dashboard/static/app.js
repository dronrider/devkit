...
// Стоит ли за строкой признак работы, любой. Работа за ней идёт, шла нашей
// сессией или кончилась вместе с ней, и во всех трёх случаях разговор у задачи
// уже есть. От этого зависит главная кнопка строки. Есть признак, значит кнопка
// продолжает работу в том же разговоре; нет признака, значит она запускает
// новую, и это самое частое действие очереди (решение пользователя).
//
// Слово talk этой функции больше не принадлежит. Оно осталось за живым
// разговором без работы (rowTalkLive и всё, что от неё), и пара понятий тут
// путалась три приёмки подряд: одно про признак работы, другое про идущий ход
// в чате (замечание ревью 12).
function rowHasRun(row) {
...
//   resume за строкой есть признак работы, и продолжение идёт в её разговор;
...
//
// Секция строки в правило не входит. Ответ считается по признакам работы, а
// подпись действия секция даёт отдельно, через actionLabel. Параметр секции тут
// стоял и не читался, а два экрана называют её по-разному: список ключом
// секции, форма полем строки (замечание ревью 12).
function rowActionKind(row) {
...
function rowAction(project, row, sect) {
  ...
  // это общим правилом (rowActionKind): полоса действий задачи выбирает кнопку
  // тем же вопросом, и разойтись им теперь негде.
  const kind = rowActionKind(row);
...
function taskActions(project, id, row) {
  ...
  if (kind === "resume") {
    ...
    out.push(withTip(go, resumeTip(label)));
    return out;
  }
...
// Форма задачи вместо экрана записи: та же подмена, что и в обратную сторону, и
// по тем же причинам. Дорога сюда одна: груминг увёл запись из накопителя
// заведённой задачей, пока форма была открыта (DK-719).
function goTaskInstead(project, id) {
  const chat = route().chat;
  const to = "#" + project + "/" + id + (chat ? "/chat/" + chat : "");
  if (location.hash !== to) history.replaceState(history.state || {}, "", to);
  return refresh();
}
...
function inlineCodeSpan(raw, where) {
  ...
  // Путь документа репозитория и ID задачи это исключение: в кавычках их пишут
  // и агенты, и человек, и это имя документа или задачи, а не команда, так что
  // ссылка там ожидаема (замечание пользователя по снимку, а после DK-1120
  // жалоба на ID, ставший фрагментом копирования без дороги к задаче). Вид
  // кода при этом остаётся, ссылкой становится обёртка. Команда с ID внутри
  // («taskctl show DK-397») остаётся командой: она не одно слово.
  const named = /\.md$/i.test(said) || /^[A-Za-z][A-Za-z0-9]*-\d+$/.test(said);
  const addr = where && named ? mentionAddr(where, said) : "";
...
function chatBubble(who, text, meta, tip, quote) {
  ...
  // Ограждение кода вокруг блока снимается тут же, одной подготовкой на разбор
  // и на рисовку: иначе номера строк у них разошлись бы (DK-892).
  wrap.mdText = askUnfence(text);
  const bb = el("div", "bb");
...
async function chatState(project, addr, board, works) {
  ...
  // Адрес склеенного захода ведёт на его строку (DK-723). Заходы одного окна
  // это один разговор, своей строки у склеенного захода больше нет, и старая
  // ссылка либо память вкладки открывают голову с полной историей. Заход
  // старше потолка склейки в поле past не попадает, стоит в списке своей
  // строкой и открывается сам собой.
  if (st.sid && !st.entry) {
...
// бы его в той же строке. Спрашивается только у чата с полем titleNamed:
// его заголовок поставил дашборд или человек (custom-title, флаг --name),
// а не эвристика по тексту первой реплики. Без этой сверки свободный чат со
// случайно совпавшим ai-title терял бы чип задачи по одному текстовому
// совпадению (замечание ревью). Пусто, значит заголовок номера не несёт
// либо назван не дашбордом, и все чипы задач рисуются как обычно.
function chatTitleTask(c) {
  if (!c || !c.titleNamed) return "";
  const m = /^([A-ZА-Я][A-ZА-Я0-9]*-\d+)\b/.exec(c.title || "");
  return m ? m[1] : "";
}
...
function chatOption(project, c, current, done) {
  ...
  // Живой чат различается занятостью: работает агент или ждёт реплики. У
  // занятого рядом идут минуты хода (DK-893): без них «активна» одинаково
  // выглядит и у хода, начатого секунду назад, и у получасового, а открывать
  // разговор ради этого дорого.
  const busyNow = c.state === "live" && !c.idle;
  chips.append(el("span", "chip" + (busyNow ? " c-run" : ""),
    busyNow
      ? LIVE_WORD.busy + (c.sec ? ", " + turnMinWord(c.sec) : "")
      : CHAT_STATE_WORD[c.state] || c.state));
...
// архив: свой список задачи, если задачу назвал адрес панели, иначе весь
...
//
// Сужение смотрит на адрес, а не на st.task: задача приезжает и от самого
// разговора (поле tasks), а она есть почти у каждого. У задачи обычно один
// чат, замены среди её чатов не находилось, и панель уходила в «Новый чат»
// при полном списке проекта (замечание пользователя, разбор DK-970).
// Кандидаты берутся из st.chats, а не из chatVisible: тот сам режет список по
// st.task, пока включён тумблер фильтра, а включён он по умолчанию.
function chatPanelNext(project, st, goneId) {
  const only = chatIsTask(st.addr) ? st.addr : chatNewTask(st.addr);
  const scoped = only ? st.chats.filter((c) => (c.tasks || []).includes(only)) : st.chats;
...
function chatDropOpen(project, st, anchor, again) {
  ...
  let loading = false;
  // Перечитывание списка тем же окном идёт своим счётом: fresh сторожит от
  // двойного запроса, а rewarm держит слова о перечитке, пока ответ в полёте.
  // Первый взгляд на список рисуется из состояния слота, и без строки человек
  // не отличит подтверждённый состав от памяти вкладки. Открытие с again
  // перечитку не заводит и строки не пишет.
  let fresh = false;
  let rewarm = !again;
  ...
  const draw = () => {
    ...
    // Перечитка этого открытия ещё в полёте: строка под строками говорит о
    // ней, пока ответ не приехал.
    if (rewarm) rows.append(el("div", "hint", "список перечитывается..."));
  ...
  const refresh = () => {
    ...
    }).catch((e) => { fresh = false; rewarm = false; console.error(e); });
  };
...
function chatWay(st) {
  ...
  // Снятый и пересозданный разговор (поле gone) своей дороги тут больше нет.
  // Имя tmux-сессии реестр отдал другой записи, процесса у разговора не
  // осталось, и реплика едет обычной дорогой резюма: ручка /say поднимает
  // claude --resume под свежим именем, история продолжается, ответ приходит в
  // ту же ленту. Прежде панель гасила отправку и ставила пузырь «не
  // доставлено» с повтором и отменой, хотя сервер эту дорогу уже умел
  // (DK-1011). На сервере поле остаётся следом перезапуска работы.
...
// остаются часы с минутами.
function turnWord(sec) {
  const s = Math.max(0, Math.round(sec));
  if (s < 60) return s + " с";
  const mins = Math.floor(s / 60);
  if (mins < 60) return mins + " мин " + (s % 60) + " с";
  return Math.floor(mins / 60) + " ч " + (mins % 60) + " мин";
}

// Тот же возраст для строки списка: там ход подписан минутами. Секунды в
// списке только мельтешили бы, а список читают глазом по всей высоте.
function turnMinWord(sec) {
...
// Час и минута местных часов. localTime тут не годится: он спрашивает время у
// раскладки браузера, и на английской машине плашка сказала бы «11:19 PM», а
// человек в своих записях и в терминале видит 23:19.
function clockWord(at) {
  const d = new Date(at);
  if (!at || isNaN(d.getTime())) return "";
  return String(d.getHours()).padStart(2, "0") + ":" + String(d.getMinutes()).padStart(2, "0");
}

// Слова плашки о ходе: имя того, с кем говорит человек, и что он делает. Точек
// на конце нет, потому что следом идёт счётчик времени, и «думает..., 6 мин»
// читалось бы обрывком.
const BUSY_WORD = "агент работает";
...
//
// Плашка это состояние чата, а не след нажатия (DK-893). Панель следит за
// состоянием с открытия и держит плашку, пока сервер говорит «занят»: ход,
// начатый до открытия панели, в чужом окне или до перезагрузки страницы, тоже
// её получает. Возраст хода приезжает тем же ответом (метка смены состояния в
// реестре клиента), и по нему в плашке идёт секундный счётчик.
function makeBusy(project, box) {
  const row = el("div", "busyrow");
  row.hidden = true;
  const dot = el("span", "dot pulse");
  const what = el("span", "busytext", BUSY_WORD);
  row.append(dot, what);
  box.append(row);
  let poll = null;
  let beat = null;
  ...
  let shownAt = 0;
  // Сессия, за состоянием которой панель следит, пока открыта.
  let watched = "";
  // Чем агент занят и когда начался ход. Слова приходят из ленты, время с
  // сервера, а рисуются они одной строкой: «думает, 6 мин 12 с».
  let word = BUSY_WORD;
  let since = 0;
  // Плашка держит своё слово, а не слово опроса. «keep» это состояние, которое
  // опросу не перебить (подъём, лечение, остановленный разговор, пропажа);
  // «poll» это дожим стопа, который опрос как раз и снимает, застав сессию
  // настоящей idle (DK-716).
  let hold = "";
  // Ход кончен явно (стоп, причина на пузыре): занятость, которую сервер ещё
  // видит по хвосту транскрипта, плашку обратно не поднимает, пока сессия хоть
  // раз не скажется свободной.
  let mute = false;
  const draw = () => {
    if (row.hidden || hold) return;
    what.textContent = word + (since ? ", " + turnWord((Date.now() - since) / 1000) : "");
  };
  ...
  // Слежение снимается уходом с панели, а не гашением плашки: пока разговор на
  // экране, следующий ход поднимет её сам, хоть начат он отсюда, хоть из
  // терминала на другой машине.
  const shut = () => {
    off();
    mute = false;
    watched = "";
  ...
  // миллисекундами, ноль значит, что времени начала нам не сказали (клиент без
  // метки), и плашка тогда горит без счётчика.
  const lift = (at) => {
    hold = "";
    row.hidden = false;
    row.classList.remove("stop");
  ...
  // Процесс пропал посреди хода: реестр помнит «busy», а сессии за ним больше
  // нет. Слова тут другие, чем у размышления и у остановленного разговора:
  // человек ждал ответа и должен знать, что ждать больше нечего.
  const gone = (at) => {
    hold = "keep";
    mute = false;
    row.hidden = false;
    ...
    since = 0;
    beatOff();
    const when = clockWord(at);
    what.textContent = "агент пропал" + (when ? " в " + when : "") +
      " посреди хода: реплика поднимет разговор снова";
  };
  const apply = (b) => {
    if (b.gone) return gone(b.at);
    if (!b.live) {
      // Процесса нет, и хода нам никто не обещал: плашке гореть не о чем.
      // Остановленный разговор и подъём сессии тут не трогаются, они и стоят
      // на том, что процесса нет.
      if (hold !== "keep") off();
      // Молчаливый стоп снимается и тут. Иначе разговор, поднятый заново из
      // терминала, доставался бы панели без плашки: гасить в нём нечего, ход
      // там ещё не начинался.
      mute = false;
      return;
    }
    if (!b.busy) {
      ...
      if (shownAt && Date.now() - shownAt < 6000) return;
      if (hold !== "keep") off();
      // Сессия сказалась свободной честно: молчаливый стоп больше не нужен,
      // следующий ход плашку поднимет.
      mute = false;
      return;
    }
    // Занятость подтверждена: потолок молчания отодвигается, и плашка гаснет
    // теперь только там, где замолчал сам опрос, а не по своему сроку.
    stop = Date.now() + LIMIT;
    if (hold || mute) return;
    const at = typeof b.sec === "number" ? Date.now() - b.sec * 1000 : 0;
    // Начало хода переставляется только на настоящем расхождении: секунды
    // приезжают целыми, и мелкая разница дёргала бы счётчик туда-сюда.
    if (row.hidden || !since || !at || Math.abs(at - since) > 2500) lift(at || since);
  };
  // Индикатор не висит вечно ни при какой поломке, но срок его меряется
  // молчанием опроса, а не длиной хода (DK-893). Ход длиннее срока плашку не
  // гасит: каждый ответ «занят» отодвигает срок, и гаснет она там, где сервера
  // не слышно вовсе.
  ...
  return {
    ...
    on(sid) {
      ...
      mute = false;
      word = BUSY_WORD;
      // Ход начался нажатием, и начало его известно без сервера: счётчик
      // пойдёт с нуля, а первый же ответ опроса его выправит.
      lift(Date.now());
      stop = Date.now() + LIMIT;
      shownAt = Date.now();
    ...
    saw(item) {
      ...
      if (item.role === "thinking") { word = "агент думает"; draw(); return; }
      if (item.role === "tool") {
        word = (item.tool || "инструмент") + (item.note ? ": " + item.note : "");
        draw();
        return;
      }
      if (item.role === "toolout") { word = "агент читает вывод"; draw(); }
    },
    off,
    shut,
  };
...
// дожимается сама. Кнопок в пузыре нет вовсе (DK-1011): дожим это дело
// дашборда, а не человека. Телефон теряет связь в метро и в лифте, и реплика,
// набранная там, уходила в никуда, стоило закрыть вкладку.
//
...
function echoRead(project, addr) {
  try {
    ...
    return raw.filter((m) => m && m.text).map((m) => ({
      ...
      // Очередь и задача переживают перезагрузку вместе с пузырём: реплика,
      // легшая во вход задачи, обязана и после неё называть очередь, а не
      // недоставку. Прежде оба поля писались в персист и не читались из него.
      queue: Boolean(m.queue),
      task: m.task ? String(m.task) : "",
...
// раз, когда клиент умер на входе. Третьего захода нет, иначе умирающий
// клиент крутил бы подъёмы без конца.
const CHAT_LIFT_TRIES = 2;

// Слова у реплики, которую увёз с собой умерший клиент подъёма. Запасные: свою
// причину смерти панель читает памятью чата (chatDeadOf).
const CHAT_LIFT_DEAD_WHY = "сессия подъёма умерла дважды подряд";
  ...
  try {
    ...
    const keep = list.filter((m) => m.state === "bad" || m.state === "held" || m.state === "wait")
      .map((m) => ({ text: m.text, wire: m.wire, born: m.born, state: m.state,
        ...
        queue: Boolean(m.queue), task: m.task || "" }));
    if (keep.length) {
...
function makeEcho(project, box, feedBox, addr, resend) {
  ...
  const out = {
    ...
    held(m, why, task) {
      ...
      m.why = why;
      m.queue = false;
      // Задача едет в персист вместе с пузырём: перезагрузка страницы не
      // должна уносить память о том, где реплика лежит.
      if (task !== undefined) m.task = task || "";
      save();
      draw();
      if (out.onHeld) out.onHeld();
    },
    // Реплика доехала до очереди, которую разберут без человека: вход задачи,
    // очередь харнеса на сбросе подписки, очередь вставшего клиента. Пузырь
    // называет очередь одной строкой и стоит, пока эхо из транскрипта его не
    // снимет. Дожима тут нет: реплика уже лежит, и второй копии не нужно.
    queued(m, why, task) {
      if (!mine.includes(m)) return;
      m.state = "held";
      m.why = why;
...
function chatPanel(project, st) {
  ...
  const post = (text, sel, pic, key, took) => {
    ...
    // Подъём сессии первой репликой. Умершая сессия подъёма это повод поднять
    // её ещё раз самой панелью, а не кнопка человеку: клиент умирает на входе
    // (истёкшая подписка, занятая машина), и второй заход поднимает его без
    ...
    const lift = (wire, tries) => {
      chatRaise(project, st, wire, st.entry ? st.entry.model : chatModelPref(),
        ...
        .then((got) => {
          ...
          if (got === "dead") {
            // Реплика уехала клиенту аргументом запуска и умерла вместе с ним.
            const dead = chatDeadOf(project, st.addr);
            if (tries < CHAT_LIFT_TRIES) {
              lift(wire, tries + 1);
              return;
            }
            echo.held(m, (dead && dead.why) || CHAT_LIFT_DEAD_WHY);
            busy.off();
    ...
    // первой строкой: у всех дорог он один и тот же.
    const go = (wire) => {
      if (way.kind === "task") {
        ...
        answerTask(project, st.task, wire)
          .then((got) => {
            ...
            // Ведущей сессии у задачи нет: строка лежит во входе задачи и
            // ждёт её. Это доставка в очередь, и пузырь называет очередь
            // одной строкой: сессия задачи заберёт строку первым же ходом, а
            // ждущую строку доски будит сторожок тиком. Прежде тут стояло
            // «доставлено», пузырь снимался через десять секунд, и реплика
            // уходила безадресной в чужой ход (живой случай DK-466).
            if (got.undelivered) echo.queued(m, got.why || taskQueueWhy(st.task), st.task);
            else echo.sent(m);
          })
  ...
  const arm = (again) => {
    ...
    // Слежение за состоянием чата (DK-893). Плашка встаёт по ответу сервера, а
    // не по нажатию «Отправить»: открытие чата, перезагрузка страницы и заход с
    // другого устройства посреди чужого хода показывают её сразу. Слежение
    // поднимается и при возврате в разговор: уход с панели его снял.
    if (st.sid) busy.watch(st.sid);
...
// Со скольких кругов опроса без разобранного блока панель говорит про это
// словами (DK-892). Круги считаются не от признака, а от новой реплики в
// ленте: признак пишет сама команда `decide --chat` посреди хода, а реплику с
// блоком агент печатает концом хода, и между ними проходит не три секунды, а
// минута с лишним. Порог держит и тот случай, когда ход кончился двумя
// пузырями подряд и блок приехал вторым: двенадцать секунд ему хватает.
const ASK_BLIND = 4;
...
// askBlindCount считает круги опроса, на которых признак стоит живым, а блока
// вопроса в ленте нет (DK-892). Порог сам по себе тут не годится. Признак
// пишет команда `decide --chat` посреди хода, реплика с блоком приезжает
// концом хода, и всякий штатный вопрос успел бы мигнуть подсказкой. Поэтому
// счёт начинается не раньше, чем в ленте появится новая реплика: пока агент
// говорит, ждать ему никто не мешает. Признак, который панель нашла уже
// стоящим (человек открыл разговор с вопросом), ждать новой реплики не
// заставляет: реплика пришла до открытия, и блок в ней либо есть, либо нет.
function askBlindCount(feed, pick, ask) {
  const sign = ask ? JSON.stringify(ask) : "";
  const said = feed ? (feed.querySelectorAll(".msg") || []).length : 0;
  if (sign !== pick.ask) {
    // Первый снимок панели про прошлое ленты ничего не знает, и реплику с
    // блоком он уже застал; всякий следующий признак завёлся на глазах.
    pick.grown = pick.ask === undefined;
    pick.ask = sign;
    pick.mark = said;
    pick.blind = 0;
  }
  if (!pick.on || pick.seen) {
    pick.blind = 0;
    return;
  }
  if (said > pick.mark) pick.grown = true;
  pick.blind = pick.grown ? (pick.blind || 0) + 1 : 0;
}

// Подсказка вместо молчания (DK-892): признак ожидания живой, а блока вопроса в
// последней реплике нет. Так бывает, когда агент завернул блок вконец или
// пересказал его своими словами. Кнопок тут нет нарочно: ответ уезжает обычной
// репликой, той же дорогой, что при галочках, а панель называет вопросы словами
// признака и говорит, чем на них отвечать.
//
// Вопросов в признаке бывает до четырёх, и едут они полем `steps`; первый из
// них ручка дублирует в `text` и `options` (agentAskOf в waiting.go). Подсказка
// печатает всю пачку: промах DK-891 случился как раз на четырёх развилках, и
// человек, которому показали одну, ответил бы на одну.
function askBlindSteps(ask) {
  const steps = (ask.steps || []).filter((step) => step && (step.text || (step.options || []).length));
...
function paintAgentBlind(box, ask) {
  ...
  if (left) head.append(el("span", "n", left === "срок вышел" ? left : "осталось " + left));
  box.replaceChildren(head);
  const steps = askBlindSteps(ask);
  const many = steps.length > 1 ? "Вопросов " + steps.length + ", а блока" : "Блока";
  box.append(el("div", "caskhint",
    many + " вопроса в последней реплике нет, и галочек при вариантах не будет. " +
    "Ответьте обычной репликой в чат: «имя развилки и номер варианта», " +
    "своими словами или «по рекомендации»."));
  const words = el("div", "casks");
  steps.forEach((step) => {
    words.append(el("div", "", step.text || "заход спросил и ждёт"));
    (step.options || []).forEach((opt, i) => {
      words.append(el("div", "caskflat", i + 1 + ". " + (opt.text || "")));
    });
  });
  box.append(words);
}
...
function paintClientAsk(project, st, box, ask, again, pick) {
  ...
  if (ask && ask.kind === "agent") {
    // Блок разобран, галочки стоят при вариантах, и говорить тут больше нечего.
    if (!blind) {
      box.hidden = true;
      box.replaceChildren();
      return;
    }
    // Блока в последней реплике нет, а заход стоит и ждёт. Прежде панель это
    // молчала, и от штатной работы случай не отличался: человек видел вопрос
    // без галочек и не знал, ждут от него ответа или нет (DK-892).
    box.hidden = false;
    paintAgentBlind(box, ask);
    return;
  }
...
// Строка ограждения кода: тройные обратные кавычки, с языком после них или без.
const ASK_FENCE_RE = /^\s*```/;

// askUnfence снимает ограждение кода вокруг блока вопроса (DK-892). Агент
// вставляет блок в реплику тройными кавычками, хотя правила говорят обратное:
// общие правила письма харнеса кладут сниппеты в ограждение и перевешивают.
// Рисовка собирает ограждённое одним узлом pre, пунктов li с data-mdline там не
// появляется, и галочкам сесть некуда, а человек читает вопрос портянкой.
// Строки ограждения гасятся на месте, пустыми: номера строк блока остаются
// теми же, и разбор с рисовкой сходятся на одном тексте. Гасится только то
// ограждение, внутри которого и правда лежит блок вопроса, поэтому сниппет
// команды в той же реплике остаётся кодом.
function askUnfence(text) {
  const lines = String(text || "").split("\n");
  let open = -1;
  const drop = (at, to) => {
    if (!askChatPicks(lines.slice(at + 1, to).join("\n")).length) return;
    lines[at] = "";
    if (to < lines.length) lines[to] = "";
  };
  for (let i = 0; i < lines.length; i++) {
    if (!ASK_FENCE_RE.test(lines[i])) continue;
    if (open < 0) {
      open = i;
      continue;
    }
  ...
  // Незакрытое ограждение рисовка тянет до конца реплики, и блок внутри него
  // пропадает тем же порядком.
  if (open >= 0) drop(open, lines.length);
  return lines.join("\n");
}
...
function askPickWire(feed, ta, state) {
  ...
  // Нашёлся ли блок, помнит сама память галочек: по этому признаку панель
  // говорит словами, что вопрос стоит, а разобрать его не вышло (DK-892).
  state.seen = Boolean(at);
  if (!at) {
    askPickPut(ta, state, "");
    return;
  }
...
async function paintChat(project, addr, board, works) {
  ...
  if (kept) {
    ...
    if (kept.arm) {
      ...
      // Список разговоров слота помнит прошлую сборку, а сосед родился или
      // закрылся мимо вкладки. Перечитка начинается сама, тем же окном, и
      // ложится в состояние слота, пока человек ведёт курсор к шапке: первый
      // взгляд на список тогда видит настоящий состав, а не память вкладки
      // (жалоба пользователя с шагами).
      if (kept.st) chatLoadWindow(kept.st.project, kept.st, kept.st.days).catch(console.error);
      return;
    }
  ...
  // Слот помнит, чем поднять своё живое, что в нём стоит и каким состоянием
  // он собран: возврат в этот разговор обходится показом и подъёмом, без
  // похода в сеть за состоянием. Перечитка списка разговоров на возврате
  // берёт это же состояние и идёт в фон, не задерживая показ готового узла.
  slot.arm = chatArm;
...
function findInput(cls, q) {
  ...
  wireFindField(input, clear, true);
...
// шапке и на экране выдачи, и содержимым поля с экрана выдачи не уводят:
// набор там держит клавиатуру, ушедшую вместе с телефонным экраном при любой
// паузе между буквами (DK-1041). У поля шапки короткий и пустой запрос
// по-прежнему возвращают доску (DK-397), оно живёт вне списка и переход
// переживает. Третий аргумент отличает поле экрана выдачи.
function wireFindField(input, clear, screen) {
  ...
  input.addEventListener("input", () => {
    showClear();
    findType(input.value, screen);
  });
  input.addEventListener("keydown", (ev) => {
    // Escape это отказ от поиска у поля шапки: поле пустеет, и экран
    // возвращается к тому, что было под выдачей. У поля экрана выдачи
    // findGo с тем же screen никуда не уводит, запрос просто стирается.
    if (ev.key === "Escape") {
      reset();
      return;
    }
...
function findType(value, screen) {
  clearTimeout(findTimer);
  findTimer = setTimeout(() => { findGo(value, screen); }, FIND_WAIT);
}
...
// Второй аргумент true у поля экрана выдачи: набор и стирание там держат
// экран при любой длине запроса, а короткое и пустое поле отвечает
// подсказкой сервера про два символа, а не уходом на доску (DK-1041). Уход с
// выдачи на телефоне остаётся за крошкой «Доска» и кнопкой «назад».
function findGo(value, screen) {
  const rt = route();
  ...
  if (!project) return;
  const q = String(value).trim();
  // Пустое поле шапки это отказ от поиска, а не поиск пустоты: экран выдачи
  // уходит целиком и открывается доска. Прежде адрес оставался «.../find/»,
  // выдача висела прежними строками, а следом за ней вставала пустая с
  // надписью про два символа, и лечилось это только перезагрузкой (замечание
  // пользователя). Короче двух знаков это ещё не запрос: сервер такой не
  // ищет и отвечает словами про два символа. У поля экрана выдачи это не
  // отказ, а ожидание нового запроса: содержимое поля не должно решать за
  // человека, что он закончил искать (развилка «правило» DK-1041), и ветка
  // ниже для него не выполняется.
  if (!screen && Array.from(q).length < FIND_MIN) {
...
async function renderDraft(project, works, id) {
  ...
  // Груминг кончился заведением задачи, пока форма была открыта: записи в
  // накопителе больше нет, и формы черновика для этого ID тоже. Экран уходит на
  // форму задачи, как экран задачи уходит на запись в обратную сторону. Правка
  // в поле тут не сторож: файл, которому она предназначалась, уже унесён
  // разбором, и держать её на экране значило бы обещать сохранение, которого не
  // будет.
  if (!text.ok && text.body && text.body.task) {
    if (taskDraft.id === id) { taskDraft.dirty = false; taskDraft.edit = false; }
    return goTaskInstead(project, text.body.task);
  }
