...
function harnessRow(h) {
  ...
  // насколько всё плохо (замечание 21 двенадцатого круга POC).
    ...
    // Возраст снимка стоит и тут: подписку выбирают по остатку, и знать,
    // насколько он свеж, надо ровно в этом месте.
...
const TBL_COLS = {
  tasks: [
    ...
    // состояния, и ширина колонки этот отступ включает. Числа тут выросли
    // вместе с боковым отступом ячейки (пять точек стали двенадцатью): место
    // под собственную подпись колонка обязана иметь при любом отступе, и
    // сторожит это TestBoardTableLabelsNotCut.
    { key: "id", label: "Номер", by: "номеру", first: "asc", w: 96 },
  ...
  drafts: [
    ...
    // хватало на собственное содержимое, и «средний» читался «средн».
    { key: "prio", label: "Приоритет", by: "приоритету", first: "desc", w: 140 },
