...
// словами. Реплика с блоком приходит позже самого признака, и подсказка с
// первого круга мигала бы на каждом штатном вопросе (DK-892).
const ASK_BLIND = 2;
...
function watchClientAsk(project, st, box, feed, ta, pick) {
  ...
  const tick = async () => {
    ...
    try {
      ...
      // Круги опроса без разобранного блока считаются, и подсказка встаёт со
      // второго. Реплика с блоком приходит позже самого признака, и подсказка
      // с первого круга мигала бы на каждом штатном вопросе.
      pick.blind = pick.on && !pick.seen ? (pick.blind || 0) + 1 : 0;
...
// репликой, той же дорогой, что при галочках, а панель называет вопрос словами
// признака и говорит, чем на него отвечать.
function paintAgentBlind(box, ask) {
  ...
  box.append(el("div", "caskhint",
    "Блока вопроса в последней реплике нет, и галочек при вариантах не будет. " +
    "Ответьте обычной репликой в чат: «имя развилки и номер варианта», " +
    "своими словами или «по рекомендации»."));
  const words = el("div", "casks");
  words.append(el("div", "", ask.text || "заход спросил и ждёт"));
