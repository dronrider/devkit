...
const TBL_COLS = {
  tasks: [
    ...
    // состояния, и ширина колонки этот отступ включает. Числа тут ходили за
    // боковым отступом ячейки: с пяти точек он ушёл в двенадцать, оттуда в
    // восемь, и колонки следом ужались на разницу. Место под собственную
    // подпись колонка обязана иметь при любом отступе, а лишнего держать не
    // должна: и то, и другое меряет TestBoardTableLabelsNotCut.
    { key: "id", label: "Номер", by: "номеру", first: "asc", w: 92 },
    { key: "title", label: "Задача", by: "названию", first: "asc", flex: true },
    { key: "rank", label: "Ранг", by: "рангу", first: "desc", w: 64 },
    { key: "date", label: "Дата", by: "дате", first: "desc", w: 84 },
  ...
  sess: [
    ...
    { key: "live", label: "Ход", ico: "i-pulse", by: "ходу работы", first: "asc", w: 44 },
    ...
    { key: "moved", label: "Активность", by: "последней активности", first: "desc", w: 116 },
  ...
  drafts: [
    ...
    // хватало на собственное содержимое, и «средний» читался «средн». Двадцать
    // точек колонка вернула названию, когда с её первой ячейки сняли отступ под
    // кружок, которого у накопителя нет вовсе.
    { key: "prio", label: "Приоритет", by: "приоритету", first: "desc", w: 120 },
    { key: "id", label: "Номер", by: "номеру", first: "asc", w: 76 },
    { key: "title", label: "Задача", by: "названию", first: "asc", flex: true },
    { key: "date", label: "Дата", by: "дате", first: "desc", w: 84 },
...
function renderBoard(project, board, works) {
  ...
  // Конец работы доска замечает сама, тем же опросом, каким экран записи ждёт
  // исхода груминга: строка под «Стопом» иначе стоит так до перезагрузки
  // страницы, а разбор черновика кончается тихо (замечание пользователя).
  // Круг заводит только tmux-работа: у неё и стоит «Стоп», который надо снять.
  if ((board.sections || []).some((sec) => (sec.rows || []).some((row) => row.run === "tmux"))) {
    watchRunning();
  }
...
function modelPick(project, st) {
  ...
  // Селектор там не действие, а честный текст: подписку называет подсказка.
  // Отдельной метки с именем подписки рядом не стоит: имя модели и так
  // называет подписку однозначно, а метка выглядела кнопкой и путала.
...
// механикой, какой панель разговора дожидается доставки реплики. Зовут его два
// экрана: запись накопителя ждёт исхода разбора, а доска ждёт конца работы у
// своих строк. Груминг идёт живым чатом, и его сессия переживает конец разбора:
// без опроса строка стояла бы под «Стопом» до перезагрузки страницы (замечание
// пользователя). Таймер один: следующий заводит очередная перерисовка, пока
