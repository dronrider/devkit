...
function taskActions(project, id, row, works) {
  ...
  if (row.run === "other") {
    out.push(el("span", "hint", "Задача в работе на другой машине: на этой у неё нет " +
      "ни одной сессии, запускать её отсюда нечем. Поговорить о ней можно: " +
      "окно чатов в шапке открывается с фильтром по задаче, и новый разговор " +
      "поднимается у нас, конвейера он не трогает."));
    return out;
  }
