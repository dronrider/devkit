...
// при возврате на разговор (feedPlace).
async function wireFeed(project, sid, opts) {
  ...
  // верху. Коробка прокрутки переживает переключение вкладок разговора
  // (тот же tp.body у соседних сессий), поэтому слушатель снимается сам при
  // уходе с ленты, иначе он копился бы с каждым переключением.
  const onScroll = () => {
    if (gone()) return;
    feedPlace.set(sid, { bottom: atBottom(scroll), rest: scroll.scrollHeight - scroll.scrollTop });
  ...
  // иначе прежнее место из feedPlace. Хвост подаёт только последние реплики,
  // и дальняя история после возврата подгрузится тем же движением прокрутки,
  // что и всегда.
  draw();
  const place = feedPlace.get(sid);
  if (place && !place.bottom) keepPlace(scroll, place.rest);
...
// оборвала бы хвост на каждом обновлении экрана. Класса tmuxbar на ней нет,
