...
async function paint() {
  ...
  if (!list.ok) {
    ...
    // Есть ли что терять, спрашивается у самого экрана: shownScreen к этому
    // моменту уже стоит текущим обходом и о нарисованном не говорит ничего.
    if (document.getElementById("groups").children.length) {
      sayResult("список проектов не прочитался: " + why, true);
      return;
    }
    headName("Связи нет");
    document.getElementById("psub").textContent = "";
    showError("список проектов не прочитался: " + why +
      ". Это про связь с сервером, а не про настройку: доски на месте, экран вернётся сам.");
    return;
  }
