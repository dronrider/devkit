...
// Идёт ли доске опрос: хватает одной строки, которая может двинуться сама.
...
// Слова исхода подъёма. Дорогу называет ручка полем way, и панель повторяет за
// ней ту же, какой реплика уехала на самом деле. Прежде кнопка говорила
// «разговор поднят резюмом» на любой ответ ручки, и в живом случае DK-673
// человек читал успех подъёма, пока реплика уезжала клавишами в чужое окно:
// лента не менялась, строки «чат продолжен резюмом» в журнале не было ни разу,
// и узнать о подмене было нечем.
function chatLiftWord(body) {
  const said = body || {};
  switch (said.way) {
    case "resume":
      return "разговор поднят резюмом: сессия встаёт";
    case "start":
      return "разговор поднят: сессия встаёт";
    case "send-keys":
      return "поднимать не пришлось: процесс жив, заказ подан прямо в него";
    case "socket":
      return "поднимать не пришлось: процесс жив, заказ ушёл в его канал";
    case "answer":
      return "поднимать не пришлось: заказ отпустил запертый вопрос агента";
    case "ask":
      return "поднимать не пришлось: заказ ушёл во вход ждущего агента";
    case "held":
      return said.stuck || "заказ не доставлен: агент ждёт ответа в своём окне";
    case "dup":
      return said.message || "заказ уже уехал, повтор никуда не поехал";
  }
  return said.message || "заказ ушёл, дорогу ручка не назвала";
}
...
// Может ли дашборд чинить этот разговор. Безнадёжен один случай: клиент жив
// и стоит на своём сокете, а окна за ним в реестре нет. Перезапуск ему не
// нужен (вход он возьмёт сам), и снимать с него нечего. Остальное чинится:
// мёртвый поднимется резюмом, разговор с нашим окном снимается и встаёт
// заново, а живущий в чужом терминале хотя бы пройдёт вход, и отказ
// перезапуска скажет дорогу.
