...
func TestStaticTaskNarrowWidths(t *testing.T) {
	...
	// Прежняя строка результата стояла в потоке документа и пустой была
	// свёрнута стилями (DK-284). Свёрнутой она места не занимала, а вот со
	// словами о нажатии уводила список вниз на свою высоту, и нажатие
	// выглядело промахом (приёмка DK-316). Теперь ответ приходит карточкой
	// поверх экрана, и мерится ровно это: карточка видна, а список стоит на
	// месте.
