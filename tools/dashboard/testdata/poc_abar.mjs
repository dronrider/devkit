...
// одной, значит задачу взяли в другом месте, и запускать её отсюда нечем.
{
  ...
  // приходится здесь, и чат её не трогает (замечание пользователя). Сам вход в
  // чат стоит у строки отдельной кнопкой, его держит стенд poc_alien.
  ...
  if (!dump(alien).includes("исполнителя не видно")) {
