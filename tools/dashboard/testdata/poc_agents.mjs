...
// --- стоп стоит только у своей tmux-работы ---
{
  ...
  // Почему у строки нет ни стопа, ни закрытия, сказано подсказкой. Слова тут
  // одни на все случаи чужой работы: цикл в другом месте, окно vscode и свой
  // терминал закрываются одинаково, а три разных объяснения одного бессилия
  // человек читал как три разных беды.
  if (!String(reg.title).includes("поднимал не дашборд")) {
  ...
  for (const gone of ["поднята вне дашборда", "идёт вне дашборда", "снимать нечем"]) {
    if (String(reg.title).includes(gone)) {
      fail("снятые слова про чужую работу вернулись в подсказку: " + reg.title);
    }
  }
  // Закрытия у чужой работы тоже нет, и говорит это сама кнопка: она стоит в
  // строке погашенной, а не пропадает, уступив место приписке.
  const shut = byClass(reg, "sclose");
  if (!shut) fail("у чужой работы пропала кнопка закрытия: " + dump(reg));
  if (!shut.disabled) fail("чужой работе предложено закрытие, которого нет: " + dump(reg));
  if (!String(shut.title).includes("поднимал не дашборд")) {
    fail("погашенная кнопка не объяснила, где эту работу закрывают: " + shut.title);
  }
  // У работы, чью сессию поднял дашборд, та же кнопка живая: гасит её
  // отсутствие имени tmux-сессии, а не вид работы. Хода в этой строке нет,
  // «Стоп» ей не адресован, и закрытие тут единственное действие.
  const mine = rowOf({ id: "DK-397", kind: "task", via: "tmux", sect: "in-progress",
    title: "договорившая сессия", own: true, talk: true, tmux: "task-DK-397-1",
    session: "aaaa1111-1111", live: "idle" });
  const open = byClass(mine, "sclose");
  if (!open) fail("у своей работы нет кнопки закрытия: " + dump(mine));
