...
// Блок следующей развилки: прежнюю человек уже закрыл, а эту агент завёл
// следующей и позвал `decide --chat` второй раз.
const queue = [
  "«очередь»: чем держать очередь выката?",
  "1. рекомендую: замком конвейера",
  "2. руками",
  "",
  "ответ строкой: «имя 1, имя 2», своими словами или «по рекомендации»",
].join("\n");

// Отвеченный блок и новый в одной реплике: агент напомнил закрытую развилку и
// тут же спросил следующую. Ждут ответа только о второй, и галочки при строках
// первой звали бы к ответу второй раз.
const both = [block, "", "Это закрыто, спрашиваю дальше.", "", queue].join("\n");

const at = (n) => new Date(Date.now() - (100 - n) * 60000).toISOString();
const answer = (n, text) => ({ key: "t:" + n, seq: n, role: "assistant", time: at(n), text });
// Реплика человека: её пузырь несёт класс «me», и по нему панель отсекает
// прочитанное. Всё, что выше такой реплики, человек уже закрыл ответом.
const mine = (n, text) => ({ key: "t:" + n, seq: n, role: "user", time: at(n), text });
...
// Вопрос идёт на развилку, и текст его начинается её именем в ёлочках: так
// собирает признак agentAskOf в waiting.go из перечня развилок записи, и по
// этим именам панель сходится с блоком в ленте (DK-892). Вопросов бывает до
// четырёх, едут они полем steps, а первый ручка дублирует в text и options;
// единственный вопрос steps не заводит вовсе, и имя развилки у него читается
// из text.
const askOf = (...qs) => {
  const steps = qs.map(([text, ...opts], i) => ({
    name: i + 1 + ". " + text.slice(0, 28),
    now: i === 0,
    text,
    options: opts.map((opt) => ({ text: opt })),
  }));
  const out = { kind: "agent", task: "DK-864", until, text: steps[0].text, options: steps[0].options };
  if (steps.length > 1) out.steps = steps;
  return out;
};

// Признак под блоком двух развилок: пачка из двух вопросов. Промах DK-891
// случился как раз на пачке, и подсказка обязана назвать её целиком.
const ask = askOf(
  ["«печать»: кто собирает текст вопроса в чат?", "утилита печатает блок", "агент собирает текст"],
  ["«ответ»: какой формы ответ человека?", "имя развилки и номер варианта", "только словами"],
);

// Признак об одной развилке: пачки тут нет, и имя панель читает из text.
const askOne = askOf(
  ["«печать»: кто собирает текст вопроса в чат?", "утилита печатает блок", "агент собирает текст"],
);

// Признак под блоком составов кейсов.
const askCases = askOf(
  ["«кейсы»: какие кейсы у DK-969?", "сбалансированный состав", "узкий состав", "широкий состав"],
);

// Признак о следующей развилке разговора: прежняя отвечена, ждут ответа об этой.
const askQueue = askOf(
  ["«очередь»: чем держать очередь выката?", "замком конвейера", "руками"],
);

// Признак от свободного вопроса `taskctl ask --question`: текст вопроса едет как
// есть, имени развилки в нём нет. Блока такому вопросу никто не печатал, блок
// печатает одна команда `decide --chat`.
const askFree = askOf(
  ["чем держать очередь выката?", "замком конвейера", "руками"],
);

// Признак о развилках второго блока в ленте.
const askOut = askOf(
  ["«выкат»: кто собирает текст вопроса в чат?", "катим сразу", "катим после ревью"],
  ["«срок»: какой формы ответ человека?", "до вечера", "до утра"],
);
...
// --- вопросов в ленте два: галочки идут при том, о котором стоит признак ---
{
...
// --- признак о другой развилке: прежний блок галочек не получает ---
{
  // Живой случай приёмки DK-892: человек ответил про «печать», агент завёл
  // развилку «очередь» и позвал `decide --chat` второй раз, а блока в ленте
  // ещё нет. Панель брала первый блок с конца ленты, не глядя на имена
  // развилок, и галочки садились на отвеченный вопрос. Ответа они уже не
  // ждали, а подсказка вместо них не поднималась: блок-то панель «нашла».
  now.ask = askQueue;
  const panel = await panelWith([answer(1, block)]);
  const box = boxOf(panel);
  if (picksOf(panel).length) {
    fail("галочки встали на блок отвеченной развилки: " + picksOf(panel).length);
  }
  const ta = sayOf(panel);
  if (ta.value !== "") fail("в поле ввода легла строка чужого блока: " + JSON.stringify(ta.value));
  for (let i = 0; i < 4; i++) await settleMove();
  if (!box || box.hidden) fail("панель промолчала о признаке, блока которого в ленте нет");
  if (!dump(box).includes("чем держать очередь выката?")) {
    fail("подсказка назвала не тот вопрос: " + dump(box).slice(0, 500));
  }
  now.ask = ask;
}

// --- отвеченный блок и новый в одной реплике: галочки только при новом ---
{
  now.ask = askQueue;
  const panel = await panelWith([answer(1, both)]);
  const picks = picksOf(panel);
  if (picks.length !== 2) {
    fail("галочек не две, по варианту новой развилки: " + picks.length + "\n" +
      dump(feedOf(panel)).slice(0, 700));
  }
  const ta = sayOf(panel);
  click(picks[0]);
  if (ta.value !== "очередь 1") {
    fail("отмечен вариант не той развилки: " + JSON.stringify(ta.value));
  }
  // Слова отвеченного блока в реплике остались, ушли из него только галочки.
  if (!dump(feedOf(panel)).includes("утилита печатает блок из перечня развилок")) {
    fail("отвеченный блок пропал из реплики");
  }
  now.ask = ask;
}
...
// --- признак без имени развилки: галочек нет, встаёт подсказка ---
{
  // Свободный вопрос `ask --question` блока не печатает, а отвеченный блок
  // интервью так и стоит в ленте. Прежде пустой набор имён ронял поиск к
  // первому блоку с конца, и галочки садились на закрытый вопрос: тот же
  // симптом, из-за которого приёмку вернули (замечание 4 ревью).
  now.ask = askFree;
  const panel = await panelWith([answer(1, block)]);
  const box = boxOf(panel);
  if (picksOf(panel).length) {
    fail("галочки встали по признаку без имени развилки: " + picksOf(panel).length);
  }
  for (let i = 0; i < 4; i++) await settleMove();
  if (!box || box.hidden) fail("панель промолчала о свободном вопросе без блока");
  if (!dump(box).includes("чем держать очередь выката?")) {
    fail("подсказка не назвала свободный вопрос: " + dump(box).slice(0, 500));
  }
  now.ask = ask;
}

// --- вопрос задан второй раз по той же развилке: прежний блок ни при чём ---
{
  // Человек ответил невнятно, и заход спрашивает снова по той же развилке.
  // Имена сходятся, а блок выше реплики человека уже закрыт, и варианты в нём
  // к этому времени другие (замечание 5 ревью).
  const panel = await panelWith([answer(1, block), mine(2, "первое своими словами")]);
  const box = boxOf(panel);
  if (picksOf(panel).length) {
    fail("галочки встали на блок выше реплики человека: " + picksOf(panel).length);
  }
  for (let i = 0; i < 4; i++) await settleMove();
  if (!box || box.hidden) fail("панель промолчала о повторном вопросе той же развилки");
  // Повторный блок доехал в ленту, и галочки встают при нём.
  await saidLater(answer(3, block));
  for (let i = 0; i < 3; i++) await settleMove();
  if (picksOf(panel).length !== 5) {
    fail("галочки не встали при повторном блоке: " + picksOf(panel).length);
  }
