...
// Признак ожидания под этим блоком: вопрос на каждую его развилку, и текст
// вопроса начинается её именем в ёлочках. Панель сверяет по этим именам блок в
// ленте с тем, о котором её спросили (DK-892), и признак об одной развилке под
// блоком о двух был бы не тем, что отдаёт ручка.
const ask = {
  kind: "agent",
  task: "DK-964",
  until,
  ...
  steps: [
    {
      name: "1. «печать»: кто собирает те",
      now: true,
      text: "«печать»: кто собирает текст вопроса в чат?",
      options: [{ text: "утилита печатает блок" }, { text: "агент собирает текст" }],
    },
    {
      name: "2. «ответ»: какой формы отв",
