...
// Замер идёт на настоящих таймерах, и живые потоки экрана держат цикл событий
// узла: у доски с живой работой заведён круг обновления, и досчитавший замер
// стоял бы с ним до конца времён. Уход с экрана снимает их тем же способом,
// каким это делает браузер.
sandbox.closeAgentLive();
// closeAgentLive снимает не все живые таймеры экрана: без явного выхода
// процесс всё равно не завершается сам.
process.exit(0);
