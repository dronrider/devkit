...
// уводит в сессии, не открывая разговоров. Полоска работ над доской с этого
// круга снята вовсе, и стенд сторожит её отсутствие.
//
...
// --- полоски работ над доской нет вовсе ---
//
// Она пережила два переезда и обессмыслилась на втором: сессии стоят своим
// табом с числом на нём, а строка над доской повторяла это число и уводила
// туда же вторым способом (решение пользователя).
{
  await go("#demo");
  if (dump(live).trim()) fail("над доской осталась полоска работ: " + dump(live));
  if (typeof sandbox.renderLive === "function") {
    fail("сборка полоски осталась в статике мёртвым кодом");
  }
}
