...
//
// Предмет стенда: открытый список начинается с открытого чата отдельной
// подписанной группой, под ней подписанная группа активных, дальше дни. Пока
...
// поломка сортировки.
const chats = [
  { id: CUR, project: "demo", title: "открытый чат", state: "dead",
...
// --- порядок групп: открытый чат, активные, дни ---
{
  const { rows } = open();
  const got = heads(rows);
  if (got.length !== 4 || got[0] !== "открытый чат" || got[1] !== "активные") {
    fail("порядок подписей групп не открытый+активные+дни: " + JSON.stringify(got));
  }
  const all = titles(rows);
  const rowNodes = allByClass(rows, "cdrow");
  if (!all[0].includes("открытый чат") || !String(rowNodes[0].className).includes(" on")) {
    fail("первая строка списка не открытый чат с подсветкой: " +
      all[0].slice(0, 200) + " | " + rowNodes[0].className);
  }
  if (!all[1].includes("живая сессия")) {
    fail("группа активных не сразу после открытого чата: " + JSON.stringify(all.map((s) => s.slice(0, 60))));
  }
...
console.log("poc_chatgroup: ok, открытый чат и активные стоят подписанными группами сверху, " +
