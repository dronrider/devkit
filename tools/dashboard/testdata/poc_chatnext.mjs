// Стенд переключения панели при уборке открытого чата (DK-656).
//
...
if (!curRow) fail("открытый чат в списке не отмечен: " + dump(drop).slice(0, 300));
if (!dump(curRow).includes("открытый чат")) {
  fail("отмеченная строка не про открытый чат: " + dump(curRow));
}
const put = byClass(curRow, "cdarch");
if (!put) fail("у строки открытого чата нет уборки в архив: " + dump(curRow));

// --- уборка открытого чата в архив ---
