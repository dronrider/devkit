...
// --- кончившийся разговор без задачи: поле живое, реплика уедет резюмом ---
{
  ...
  // Молчания тут нет: общий индикатор работы встаёт остановкой и говорит
  // словами, что сессии за разговором не осталось и поднимет её сама реплика
  // (DK-701). Иначе состояние неотличимо от живого.
  if (!said.includes("реплика поднимет его снова")) {
...
// --- снятый разговор с отданным именем окна: та же дорога резюма ---
// Своей дороги у снятого разговора больше нет (DK-1011): процесса за ним не
// осталось, и реплика едет тем же резюмом, что у любого кончившегося. Плашки с
// причиной над полем тоже нет: панель гасила ею дорогу реплики, а чат живой.
{
  const { way, ta, said } = await fieldOf(GONE);
  if (way.kind !== "resume") fail("дорога реплики снятого разговора не резюм: " + JSON.stringify(way));
  ...
  if (said.includes("имя окна отдано другому разговору")) {
    fail("плашка снятого разговора осталась над полем: " + said.slice(0, 300));
  }
}
