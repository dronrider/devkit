...
// --- адрес прошлого захода ведёт на склеенную строку ---
{
  const st = await sandbox.chatState("demo", PAST1, { prefix: "XR", sections: [] }, []);
  if (st.sid !== HEAD) {
    fail("панель со старого адреса не переехала на склеенную строку: " + st.sid);
  }
}
...
console.log("ок: заходы одного окна стоят одной строкой со счётом заходов, " +
  ...
  "а старый адрес ведёт на голову");
