...
// перерисовкой, а переключение диалога не мигает надписями об открытии
...
// Слова о переходе тут больше нет вовсе: переключение идёт тем же ходом (пул
// панелей), и надпись об открытии либо врала, либо мелькала зря (жалоба
// пользователя). Прежнее содержимое стоит до готового нового, и это и есть
// отклик.
if (dump(pin).includes("открывается другой разговор")) {
  fail("надпись о переходе вернулась в панель: переключение идёт тем же ходом");
}
await painting;
await settle();
