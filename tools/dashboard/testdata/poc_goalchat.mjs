...
//   задачу, а реплика ложится во «Входящие»;
//   над полем ввода не стоит плашки ни в одном виде, и пустая коробка
//   исходящих остаётся без слов (DK-957);
...
await settle();
// Плашки над полем ввода тут больше нет ни в одном виде (DK-957): человек
// читал её на каждой цели заново и закрывал крестиком.
if (text(panel).includes("Сообщение уйдёт агенту") || text(panel).includes("Идущий виток получит")) {
  fail("над полем живого витка вернулась плашка про доставку: " + text(panel));
}
...
// Стоящий цикл тоже не рисуется плашкой: реплика подписана «ждёт витка», а
// поднять виток можно уведомлением о стопе и кнопкой в строке доски.
if (text(panel).includes("Цикл цели не идёт") || text(panel).includes("Поднять виток")) {
  fail("над полем стоящей цели вернулась плашка с подъёмом витка: " + text(panel));
}
// Пустая коробка исходящих остаётся без слов, как и пустая лента.
if (text(panel).includes("во «Входящих»")) {
  fail("пустые «Входящие» снова названы словами: " + text(panel));
}
from = posts.length;
