...
// Кнопки, подпись экрана и легенда уезжают за разрыв, а селект остаётся в
// первой строке: порядок ему не назначается.
if (!/\.bhead \.bell[^{]*\{[^}]*order:/.test(narrow)) {
  fail("кнопки-действия не уехали во вторую строку: у .bhead .bell нет order");
}
if (!/\.bhead \.sub[^{]*\{[^}]*order:|\.sub,#hlegend\{order:/.test(narrow.replace(/\.bhead /g, ".bhead "))) {
  fail("подпись экрана не уехала во вторую строку");
}
...
const subcap = /\.bhead \.sub\{[^}]*max-width:calc\(100% - (\d+)px\)[^}]*text-overflow:ellipsis/.exec(narrow);
if (!subcap) fail("подпись экрана не заперта шириной с многоточием (.bhead .sub)");
...
// Вторая строка: четыре кнопки с зазорами влезают в запас подписи.
const btns = 4 * ctl + 4 * gap;
if (btns > Number(subcap[1])) {
  fail("запас подписи меньше полосы кнопок: " + btns + " > " + subcap[1]);
}

console.log("ok: узкая шапка в две строки с принудительным разрывом, селект в " +
  "первой, кнопки с подписью во второй, ширины заперты с запасом под 390 точек");
