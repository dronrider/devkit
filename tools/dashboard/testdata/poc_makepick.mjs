...
// --- форма черновика несёт только свои поля ---
{
  ...
  // Поле у черновика одно, и в нём лежит шаблон разделов: полем на каждый
  // раздел форма человека не встречает (решение пользователя).
  const areas = fields(groups);
  if (areas.length !== 1) {
    fail("полей на форме черновика " + areas.length + ", а надо одно: " +
      JSON.stringify(areas.map((n) => n.attrs["aria-label"])));
  }
  for (const want of ["### Ситуация", "### Осложнение", "### Вопрос", "### Гипотеза"]) {
    if (!String(areas[0].value || "").includes(want)) {
      fail("в шаблоне поля нет раздела «" + want + "»: " + JSON.stringify(areas[0].value));
    }
  }
...
// --- запись уезжает в свою ручку ---
{
  ...
  // Обязателен в черновике один заголовок: разделы человек правит руками, и
  // снятый раздел записи не мешает.
  write(groups, "текст черновика", "ссылка на черновик из чата не открывается\n\n" +
  ...
  // Уезжает написанное как есть: разметку разделов утилита записи узнаёт сама.
  const want = "ссылка на черновик из чата не открывается\n\n" +
    "### Ситуация\n\nссылку из чата человек открывает вручную.";
  if (last.body.text !== want) {
    fail("черновик уехал не тем, что написано в поле: " + JSON.stringify(last.body.text));
  }
  void field;
}
