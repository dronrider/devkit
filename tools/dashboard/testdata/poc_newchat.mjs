...
// Пузырь несёт только слова человека: правила заказа (план, ротация,
// отзывчивость) приклеивает сервер при подъёме, и в оптимистичном пузыре им
// делать нечего (замечание пользователя про приписки от его имени).
if (dump(panel).includes("Веди план работ")) {
  fail("в held-пузыре первой реплики видны приписки заказа");
}
