...
const postBodies = {};
const { sandbox, store, timers, moves, posted } = makeSandbox(app, (path, init) => {
  if (init && init.method === "POST") {
    postBodies[path] = init.body ? JSON.parse(init.body) : null;
...
// --- оптимистичный пузырь: три состояния и сверка эха ---
{
  ...
  echo.bad(bad);
  if (!dump(pend).includes("не ушло")) fail("отказ не назван в пузыре: " + dump(pend));
  if (dump(pend).includes("повторить")) fail("у неушедшей реплики осталась кнопка повтора");
  ...
  if (dump(pend).includes("повторить")) fail("у непринятой реплики осталась кнопка повтора");
  // Реплика, легшая в очередь, недоставленной не считается: очередь её
  // разберёт без человека, и пузырь называет очередь одной строкой (DK-1011).
  const queued = echo.add("четвёртая реплика", () => {});
  echo.queued(queued, "лимит подписки исчерпан, сброс 15:00");
  if (dump(pend).includes("не доставлено: лимит подписки")) {
    fail("реплика в очереди харнеса помечена недоставленной: " + dump(pend));
  }
  if (!dump(pend).includes("лимит подписки исчерпан, сброс 15:00")) {
    fail("очередь харнеса не названа в пузыре: " + dump(pend));
  }
}
...
// --- ответ ручки с пометкой очереди рисует пузырь недоставленным ---
{
  ...
  // Ответ дорогой socket со словами очереди: реплика взята и лежит в очереди
  // вставшего клиента, и пузырь называет очередь, а не недоставку (DK-1011).
  if (!dump(panel).includes("агент ждёт разрешения")) {
    fail("пузырь не назвал очередь клиента: " + dump(panel).slice(0, 300));
  }
  if (dump(panel).includes("не доставлено")) {
    fail("реплика, взятая сокетом в очередь, помечена недоставленной: " + dump(panel).slice(0, 300));
  }
...
// --- у пузыря есть кнопка копирования, как у блоков команд ---
// Реплику уносят в редактор и в соседний чат, а выделять её мышью из ленты
// неудобно: у длинной убегает прокрутка (замечание пользователя). Кнопка стоит
// у обоих пузырей, и своего, и агентского, и копирует текст сообщения целиком.
{
  const copied = [];
  sandbox.window.navigator = { clipboard: { writeText: (t) => { copied.push(t); return Promise.resolve(); } } };
  for (const who of ["вы", "агент"]) {
    const bubble = sandbox.chatBubble(who, "первая строка\nвторая строка", "13:05");
    const btn = deepBtn(bubble, "foldcp");
    if (!btn) fail("у пузыря «" + who + "» нет кнопки копирования: " + dump(bubble));
    if (String(btn.title) !== "Копировать") fail("кнопка не назвалась: " + btn.title);
    btn.handlers.click({ stopPropagation: () => {} });
  }
  await settle();
  if (copied.length !== 2) fail("скопировалось не оба сообщения: " + copied.length);
  if (copied[0] !== "первая строка\nвторая строка") {
    fail("скопирован не текст сообщения целиком: " + JSON.stringify(copied[0]));
  }
  // Подпись автора при этом остаётся подписью, а не уезжает в буфер.
  if (copied[0].includes("13:05")) fail("в буфер уехала подпись пузыря");
  // Пустому пузырю копировать нечего, и кнопки у него нет.
  if (deepBtn(sandbox.chatBubble("вы", "   ", ""), "foldcp")) {
    fail("у пустого пузыря взялась кнопка копирования");
  }
}
