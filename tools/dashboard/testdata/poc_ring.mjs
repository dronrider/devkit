...
// показан на экране цели блоком «Задачи цели» и этапом на экране задачи.
{
  ...
  // Своего плана у этой работы нет, и в середине встаёт ракета: живая работа
  // без плана читается как быстрое действие на один ход, а не число
  // работающих и не вставшая сессия (DK-978). Дашборд меток сторожа
  // plan-watch не читает, тут довольно того, что план пуст.
  if (byClass(wrap, "rnum")) fail("у живой работы без плана всё равно стоит число");
  if (!byClass(wrap, "rrocket")) fail("у живой работы без плана нет ракеты в середине");
...
// --- ожидание: красное кольцо, ореол и число ждущих ---
{
  ...
  // Ракета это про работу прямо сейчас, а не про ожидание ответа: у ждущего
  // кольца её нет, даже если своего плана нет вовсе (DK-978).
  if (byClass(wrap, "rrocket")) fail("у ждущего кольца встала ракета");
...
// --- молчание: сегменты серые, дуга не бежит ---
{
  ...
  if (byClass(wrap, "rrocket")) fail("у молчащего кольца встала ракета");
...
// --- пусто: кольцо без числа ---
{
  ...
  if (byClass(wrap, "rrocket")) fail("у пустого кольца встала ракета");
...
// пишет себе сам (TodoWrite). Плана нет, значит делений нет вовсе.
{
  ...
  // С планом в середине дробь, как и прежде, а не ракета (DK-978): ракета это
  // только про сессию без своего плана.
  if (byClass(wrap, "rrocket")) fail("у сессии с планом в середине встала ракета");
  const withPlan = allByClass(byClass(wrap, "rfrac"), "rnum").map((n) => n.textContent);
  if (JSON.stringify(withPlan) !== JSON.stringify(["1", "3"])) {
    fail("с планом дробь в середине не та: " + JSON.stringify(withPlan));
  }
  // Плана нет: делений нет, дорожка ровная, списка в подсказке тоже нет, а в
  // середине ракета вместо числа работающих (DK-978).
  ...
  if (byClass(bare, "rnum")) fail("без плана у живой работы всё равно стоит число");
  if (!byClass(bare, "rrocket")) fail("без плана у живой работы нет ракеты в середине");
}
...
// --- цель и задача: кольцо одинаково, шкалы нет ни у той, ни у другой ---
{
  ...
  // У цели своего плана тоже нет, и в середине та же ракета, что и у задачи:
  // разницы между ними на кольце больше не рисуется (DK-978).
  if (byClass(wrap, "rnum")) fail("у цели без плана всё равно стоит число");
  if (!byClass(wrap, "rrocket")) fail("у цели без плана нет ракеты в середине");
  if (byClass(head, "cts")) fail("строка состояния вернулась в шапку");
