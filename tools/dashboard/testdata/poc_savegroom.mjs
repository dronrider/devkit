...
// Поле формы черновика: своего класса у него нет, различается подписью для
// чтения с экрана. Поле одно, и в нём лежит шаблон разделов.
...
// Записи черновика хватает непустой первой строки: разделы человек правит в
// том же поле руками, и снятый раздел записи не мешает.
const fill = async (text) => {
  put("текст черновика", text.trim()
    ? text + "\n\n### Ситуация\n\nзамечено на прогоне.\n"
    : text);
  await settle();
};
