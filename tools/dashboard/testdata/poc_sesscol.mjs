...
//
// Отсюда предмет стенда: у колонки состояния подписи нет и ширина её меньше
...
// --- у колонки состояния нет подписи, а ширина её меньше порога ---
{
  const cell = colCell("live");
  if (!cell) fail("колонки состояния в шапке сессий нет: " + dump(head()));
  if (dump(cell).trim()) {
    fail("у колонки состояния осталась подпись «" + dump(cell).trim() +
      "»: место под неё и раздувало колонку");
  }
