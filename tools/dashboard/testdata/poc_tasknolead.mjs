...
//
// Предмет стенда: ручка говорит, что ведущей сессии нет, пузырь называет
// очередь задачи одной строкой, кнопок при нём нет ни одной (DK-1011), а текст
// человека никуда не девается и переживает таймеры панели.
//
...
// --- пузырь называет очередь, а не доставку, текст на месте ---
{
...
// --- кнопок при пузыре нет ни одной ---
{
  for (const word of ["повторить", "отменить", "поднять работу по задаче", "открыть живой чат"]) {
    if (deepBtn(panel, word)) fail("при пузыре осталась кнопка «" + word + "»");
  }
}
...
console.log("poc_tasknolead: реплика задаче без ведущей сессии стоит с очередью строкой, " +
  "кнопок при ней нет, текст цел");
