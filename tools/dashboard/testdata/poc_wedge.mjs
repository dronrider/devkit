...
// --- вопрос в терминале: плашка говорит про нечитаемый вопрос, а не зовёт в tmux ---
...
// клин, процесс снимать нельзя. Звать человека в терминал командой плашка
// больше не должна: сам вопрос панель показывает кнопками и отвечает на него
// сама, а плашка остаётся ровно на том случае, когда вопрос с панели не
// прочитался, и говорит она об этом (решение пользователя).
{
  const asks = () => ({
    addr: "DK-460", sid: "8e9c1cf9-1111", task: "DK-460", chats: [], models: [],
    fresh: false, error: "", note: "",
  ...
  if (said.includes("tmux attach")) {
    fail("плашка всё ещё зовёт в терминал командой: " + said);
  }
  if (!said.includes("не прочитался")) {
    fail("плашка не говорит, что вопрос не читается: " + said);
  }
  // Окно сессии названо словами: искать вопрос человеку всё равно где-то надо.
  if (!said.includes("chat-13")) fail("плашка не назвала окно сессии: " + said);
