...
// слева работа, дальше разговор, дальше три точки с выбором подписки и уровня,
// и состав этот от состояния строки не зависит вовсе. Три точки стоят у всякой
// строки, у идущей в том числе, только погашенными: пропадай они, ряд кнопок
// дёргался бы по колонке от строки к строке.
//
...
// этого не меняется: кнопки три и там, и там.
const row = (id, live) => `
    <tr class="trow">
  ...
  live ? ICO("rstop btn-danger") : ICO("rmain")}${ICO("rchat")}${ICO("rdots")}</span></span></td>
    </tr>`;
