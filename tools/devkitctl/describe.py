...
def map_dirs(root, member):
    ...
    («### 4.7 xr-relay: слепой транзит»), узнаёт только имя, а путь члена
    в манифесте бывает глубже имени (crates/hub), поэтому такой заголовок
    закрывает лишь члена верхнего уровня, у кого имя и есть путь; члену
    из вложенного каталога нужно имя со ссылкой. Строка таблицы
