...
"""devkitctl: обвязка devkit в проекте одной командой.
  ...
  devkitctl doctor [--fix] [-C dir]
      ...
      конфиг второй подписки в машинном слое (болванка env с endpoint и токеном,
      подстановки алиасов первой лестницы в модели подписки, DK-751, и то же
      окружение в копии окна, если она есть),
...
PACKAGER = "brew"
# Команда установки самого brew, официальная со страницы проекта. devkit её не
# зовёт: это тот же curl | sh, от которого уходит остальная доводка, и решение
# ставить менеджер пакетов на голую машину остаётся за человеком.
PACKAGER_INSTALL_CMD = ('/bin/bash -c "$(curl -fsSL '
...
# Якоря моделей подписки, на которые сворачиваются алиасы первой лестницы:
# флагман и лёгкая. Значения у них те же, что человек вписал в болванку, второй
# копии моделей в машинном слое нет.
ALT_SUB_FLAGSHIP = "ANTHROPIC_MODEL"
ALT_SUB_LIGHT = "ANTHROPIC_SMALL_FAST_MODEL"
...
MACHINE_IGNORE_ENTRIES = (
    ...
    (".devkit/ship.lock*",
     "# Замок конвейера shipctl и запись его держателя, живут только на машине.",
     "замок конвейера и запись держателя замусорят status"),
    (".devkit/goal-*",
...
def ensure_package(name, why, fix):
    ...
    if not manager:
        ...
        # знает, а придуманная команда хуже её отсутствия. Сам менеджер
        # находка называет тоже: без него ставить нечем не только этот пакет,
        # а вообще ничего из «осталось сделать».
        return ["%s; поставить нечем: пакетного менеджера %s на машине нет, поставить его "
                "самому: %s, потом %s install %s"
                % (why, PACKAGER, PACKAGER_INSTALL_CMD, PACKAGER, name)], []
    if not fix:
...
def alias_folding(env):
    ...
    Параметр model у вызова субагента клиент принимает только алиасами своей
    лестницы, и диспетчер, которому вердикт pick назвал модель подписки glm-именем,
    передаёт ближайший алиас. Подстановка сворачивает лестницу на модели подписки
    по силе яруса: лёгкий алиас в лёгкую модель, сильные во флагман. Без неё
    алиас уходит на провод как есть, и сервер подписки обслуживает незнакомое
    имя лёгкой моделью молча (DK-751). Имена алиасов читаются из профиля
    домашнего клиента, чтобы пятый алиас у клиента поднял подстановку правкой
    профиля, а не утилиты. Пустые якори подстановок не поднимают, сворачивать
    не на что, пока человек не вписал модели; про каждый пустой якорь,
    флагманский и лёгкий, говорит своя находка в check_alt_sub.
    """
    ...
    try:
        ...
        # Про битый и отсутствующий профиль говорит своя проверка доктора, тут
        # только следствие: подстановки не собрать.
        return {}
    anchors = {"mini": str(env.get(ALT_SUB_LIGHT) or "").strip(),
               "base": str(env.get(ALT_SUB_FLAGSHIP) or "").strip(),
               "pro": str(env.get(ALT_SUB_FLAGSHIP) or "").strip(),
               "max": str(env.get(ALT_SUB_FLAGSHIP) or "").strip()}
    want = {}
    for tier, anchor in anchors.items():
        alias = profile.str_of("delegate", "map_" + tier)
        if alias and anchor:
            want["ANTHROPIC_DEFAULT_%s_MODEL" % alias.upper()] = anchor
    return want


def check_alt_sub(fix, main=None, from_main=True):
    ...
    # автоматике их взять неоткуда. Подстановки алиасов при этом раскладывает
    # сама: значения выводятся из вписанных человеком моделей подписки.
    #
    ...
    # Лёгкая модель в ALT_SUB_KEYS не входит, болванка её не раскладывает
    # и значению автоматике взяться неоткуда, поэтому находка отдельная и без
    # команды переразкладки. Пустой якорь оставляет мини-ярус лестницы голым
    # алиасом, его выручает только запасное поведение сервера (DK-751).
    if not str(env.get(ALT_SUB_LIGHT) or "").strip():
        findings.append("в конфиге второй подписки %s не задана лёгкая модель (ключ %s): "
                        "лёгкий алиас параметра модели уходит на сервер голым и обслуживается "
                        "его лёгкой моделью молча (DK-751); вписать модель лёгкого яруса, "
                        "в подписке с одной моделью повторить флагман"
                        % (settings, ALT_SUB_LIGHT))
    # Подстановки алиасов первой лестницы сверяются с якорями и раскладываются
    # после прав: находка про широкие права обязана успеть прозвучать, а запись
    # подстановок держит режим файла, суженный строкой выше. Рубеж тот же, что у
    # остального машинного хозяйства: ключ виден каждой сессии подписки сразу, и
    # ехать на машину с непроверенной ветки ему нельзя.
    fold = alias_folding(env)
    bad = sorted(k for k, v in fold.items() if env.get(k) != v)
    if bad and fix and from_main:
        env.update(fold)
        doc["env"] = env
        settings.write_text(json.dumps(doc, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        fixed.append("расставлены подстановки алиасов в %s (ключи %s): алиасы, которые "
                     "клиент принимает параметром модели, сворачиваются на модели подписки"
                     % (settings, ", ".join(bad)))
    elif bad:
        findings.append("в конфиге второй подписки %s расходятся с моделями подписки ключи "
                        "подстановки алиасов %s: параметр модели у субагента клиент принимает "
                        "алиасом первой лестницы, и без подстановки незнакомый серверу алиас "
                        "обслуживается лёгкой моделью молча (DK-751); %s"
                        % (settings, ", ".join(bad), fix_hint(main, from_main, "подстановки алиасов")))
    return findings, fixed
...
def check_window_copy(root, fix):
    ...
    # Подстановки алиасов едут в копию тем же списком: окно, открытое без
    # CLAUDE_CONFIG_DIR, берёт env из настроек самой директории, и без подстановок
    # алиас из параметра модели уходил бы на провод как есть (DK-751).
    want.update(alias_folding(machine_env))
