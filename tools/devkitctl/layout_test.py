...
class ReadmeGuardTest(unittest.TestCase):
    ...
    def test_all_sixteen_entries_are_named(self):
        entries = layout.readme_entries(self.text)
        self.assertEqual(len(layout.TOP_LEVEL), 16,
                         "записей белого списка %d, а правило называет шестнадцать" % len(layout.TOP_LEVEL))
