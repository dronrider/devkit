...
class CoreBudgetTest(unittest.TestCase):
    ...
    def test_core_budgets(self):
        # Потолок держит решение DK-173.2 «ядро не растёт», и правило плана
        # работ (DK-613) уложено под него одной строкой. Команды в ней названы,
        # а порядок ведения лежит в скилле work-plan и в определениях
        # исполнителей: прогон стенда 27-work-plan зеленеет как раз на них,
        # вклада отдельного абзаца в ядре он не показал.
