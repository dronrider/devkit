...
class CoreBudgetTest(unittest.TestCase):
    ...
    def test_core_budgets(self):
        # Потолок ядра поднят с 5500 до 5700 в DK-613. Резидентным абзацем туда
        ...
        # правила: резидент вырос на три строки, а промпты машины стали короче.
        # Три строки, а не одна, потому что прогон стенда 27-work-plan показал:
        # отсылки к скиллу сессия не слушается, план кладут только названные
        # прямо команды.
        core = len(read(DEVKIT_SRC / "RULES.core.md"))
        board = len(read(DEVKIT_SRC / "RULES.board.core.md"))
        self.assertLessEqual(core, 5700, "ядро правил длиннее бюджета 5700 символов: %d" % core)
