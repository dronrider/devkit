...
dksrc=$(CDPATH= cd -- "$here/../.." && pwd)
dk="$tmp/devkit"
mkdir -p "$dk"
for d in tools kit hooks; do
    cp -R "$dksrc/$d" "$dk/"
done
cp "$dksrc/RULES.md" "$dksrc/RULES.board.md" "$dk/"
dkctl="$dk/tools/devkitctl/devkitctl.py"
...
# файл построчно.
allow=$(python3 - "$dk/tools/devkitctl" <<'EOF'
import json, sys
...
cp "$dk/kit/agents/"*.md "$home/.claude/agents/"
cp -R "$dk/kit/skills/"* "$home/.claude/skills/"
...
import sys
sys.path.insert(0, sys.argv[1] + "/tools/devkitctl")
import harness, rules
prof = harness.parse("p.toml", open(sys.argv[1] + "/kit/harness/claude-code.toml", encoding="utf-8").read())
...
pcli() { HOME="$phome" python3 "$dk/tools/devkitctl/perms.py" "$@" 2>&1; }
...
python3 - "$dk/tools/devkitctl" "$phome/.claude/settings.json" <<'EOF' || fail "--fix переставил рукописное или обошёл запрет"
import json, sys
sys.path.insert(0, sys.argv[1])
import perms
...
rm -f "$phome/.claude/settings.json"
python3 - "$dk/tools/devkitctl" "$phome/.claude/settings.json" <<'EOF' || fail "рубеж основного чекаута у прав не держится"
import os
import sys
sys.path.insert(0, sys.argv[1])
import perms
...
assert "/nowhere/main-devkit/tools/devkitctl/devkitctl.py doctor --fix" in findings[0], findings
...
# (tools/agentctl/harness_test.go). Разъедься парсеры, и один и тот же профиль
# читался бы двумя утилитами по-разному.
seen=0
for f in "$dk"/kit/harness/testdata/*.toml; do
    seen=$((seen + 1))
    base=$(basename "$f" .toml)
    if ! python3 "$dk/tools/devkitctl/harness.py" "$f" > "$tmp/report.out" 2>"$tmp/report.err"; then
        fail "парсер профилей упал на $base: $(cat "$tmp/report.err")"
        continue
    fi
    diff -u "$dk/kit/harness/testdata/$base.expected" "$tmp/report.out" > "$tmp/report.diff" ||
...
cp "$dk/kit/harness/claude-code.toml" "$tmp/claude-code.toml.bak"
printf '[detect]\n\n[rules]\nmode = "import"\n\n[delegate]\nmode = "none"\n\n[hooks]\n\n[quota]\n' \
    > "$dk/kit/harness/claude-code.toml"
out=$(HOME="$home" PATH="$cleanpath" python3 "$dkctl" doctor -C "$proj" 2>&1)
...
cp "$tmp/claude-code.toml.bak" "$dk/kit/harness/claude-code.toml"

# Юниты генератора правил: хеш маркера, поломанные маркеры, забор кода. Гоняются
# прямо по модулю, доктор до них не нужен.
python3 - "$dk/tools/devkitctl" <<'EOF' || fail "юниты генератора правил не прошли"
import sys
sys.path.insert(0, sys.argv[1])
import harness
import rules
...
python3 - "$dk/tools/devkitctl" "$tmp/depth" "$dk" <<'EOF' || fail "юниты глубины правил не прошли"
import os
import sys
...
put(os.path.join(dk, "kit", "skills", "board-test", "SKILL.md"),
...
assert "`../devkit/kit/skills/board-test/SKILL.md`" in ptr, ptr
...
cc = harness.parse("claude-code.toml",
                   open(os.path.join(dkroot, "kit", "harness", "claude-code.toml"), encoding="utf-8").read())
...
for name, board in (("thin-board.expected", True), ("thin-noboard.expected", False)):
    want = open(os.path.join(dkroot, "tools", "devkitctl", "testdata", name), encoding="utf-8").read()
...
HOME="$ghome" python3 - "$dk/tools/devkitctl" "$ghome" "$tmp" <<'EOF' || fail "юниты глобальной точки правил не прошли"
import os
import sys
...
python3 - "$dkreal/tools/devkitctl" "$tmp/cut" "$dkreal" <<'EOF' || fail "тонкий файл собран не под нарезанное ядро"
import os
import sys
...
for d in full core; do
    python3 "$dkreal/tools/devkitctl/rules.py" --layout "$d" "$tmp/lay/$d" "$dkreal/tools/obeycheck/testdata/project" \
...
for d in full core; do
    HOME="$glhome" python3 "$dkreal/tools/devkitctl/rules.py" --layout "$d" "$tmp/gllay/$d" "$dkreal/tools/obeycheck/testdata/project" \
...
cat > "$dk/kit/harness/embed-tool.toml" <<'EOF'
...
\
[skills]\
discovery = "manual"' "$dk/kit/harness/embed-tool.toml" > "$tmp/embed-tool.manual" &&
    cp "$tmp/embed-tool.manual" "$dk/kit/harness/embed-tool.toml"
...
grep -q '`../devkit/kit/skills/board-batch/SKILL.md`' "$rproj/AGENTS.md" ||
...
for s in "$dk"/kit/skills/*/SKILL.md; do
    sname=$(basename "$(dirname "$s")")
    grep -q "\`../devkit/kit/skills/$sname/SKILL.md\`" "$rproj/AGENTS.md" ||
...
grep -v 'discovery = "manual"' "$tmp/embed-tool.manual" | grep -v '^\[skills\]$' \
    > "$dk/kit/harness/embed-tool.toml"
out=$(docr --fix)
echo "$out" | grep -q 'починено: вклейка правил в AGENTS.md обновлена под devkit' ||
...
# Тексты те же, что у Go-стороны (tools/agentctl/harness.go, narrowByProject): один и
...
rm -f "$dk/kit/harness/embed-tool.toml"
...
# в самопроверке незачем. Соседний markdown без frontmatter в kit/agents/ имитирует
...
printf '# agents\n\nПроза, не определение.\n' > "$dk/kit/agents/README.md"
# То же со скиллами: скилл это директория с SKILL.md, соседняя проза в kit/skills/
# на машину уезжать не должна.
printf '# skills\n\nПроза, не скилл.\n' > "$dk/kit/skills/README.md"
mhome="$tmp/mhome"
...
[ -f "$mhome/.claude/skills/README.md" ] && fail "--fix положил на машину markdown из kit/skills/ как скилл: $out"
# Раскладка берёт kit/skills/ целиком, а не знакомый ей список: добавленный скилл
...
python3 - "$dk/tools/devkitctl" <<'EOF' || fail "юниты порогов резидента не прошли"
import sys
sys.path.insert(0, sys.argv[1])
import weigh
...
cp "$dk/kit/agents/"*.md "$whome/.claude/agents/"
cp -R "$dk/kit/skills/"* "$whome/.claude/skills/"
...
wagents=$(listing "$dk/kit/agents/"*.md)
wskills=$(listing "$dk/kit/skills/"*/SKILL.md)
...
echo "$out" | grep -q "листинг определений kit/agents/ .*$(fmt "$wagents")" || fail "листинг агентов не сошёлся: $out"
...
printf -- '---\nname: probe-agent\ndescription: %s\neffort: low\n---\n\nтело определения\n' "$probe_desc" \
    > "$dk/kit/agents/probe-agent.md"
cp "$dk/kit/agents/probe-agent.md" "$whome/.claude/agents/"
: > "$calls"
out=$(HOME="$whome" PATH="$wpath" WEIGH_CALLS="$calls" python3 "$dkctl" weigh -C "$wproj" --limit 20000 2>&1)
grown=$((wagents + 11 + ${#probe_desc}))
echo "$out" | grep -q "листинг определений kit/agents/ .*$(fmt "$grown")" ||
...
rm -f "$dk/kit/agents/probe-agent.md" "$whome/.claude/agents/probe-agent.md"
...
rm -f "$dk/kit/harness/embed-tool.toml" "$whome/.devkit/harness.local"
...
profile = harness.parse("cc.toml", open(os.path.join(dkroot, "kit", "harness", "claude-code.toml"),
                                        encoding="utf-8").read())
...
# отдельный тест).
out=$(python3 - "$dk/tools/devkitctl" "$wproj" "$dk" <<'EOF'
import sys
sys.path.insert(0, sys.argv[1])
import weigh
...
mkdir -p "$dk/kit/skills/oversized-probe"
python3 -c "
open('$dk/kit/skills/oversized-probe/SKILL.md', 'w', encoding='utf-8').write(
    '---\nname: oversized-probe\ndescription: тестовый скилл.\n---\n' + 'т' * 16500)
"
out=$(python3 - "$dk/tools/devkitctl" "$dk" <<'EOF'
import sys
sys.path.insert(0, sys.argv[1])
import weigh
...
rm -rf "$dk/kit/skills/oversized-probe"
...
dkreal=$(cd "$dk" && pwd -P)
wthome="$tmp/wthome"
out=$(HOME="$wthome" PATH="$gostub:$sys" python3 "$tmp/devkit-wt/tools/devkitctl/devkitctl.py" doctor --fix -C "$mproj" 2>&1)
echo "$out" | grep -q 'worktree ветки задачи' || fail "нет находки про worktree devkit: $out"
echo "$out" | grep -q "$dkreal/tools/agentctl" || fail "находка не отправляет в основной чекаут: $out"
...
echo "$out" | grep -q "cp $dkreal/kit/agents/review-high.md" || fail "находка про определения зовёт не в основной чекаут: $out"
# Скиллы держатся того же рубежа.
[ -f "$wthome/.claude/skills/board-batch/SKILL.md" ] && fail "--fix разложил скилл с фичеветки"
echo "$out" | grep -q "cp $dkreal/kit/skills/board-batch/SKILL.md" || fail "находка про скилл зовёт не в основной чекаут: $out"
...
echo "$out" | grep -q "$dkreal/tools/devkitctl/devkitctl.py doctor --fix" ||
...
cp "$dk/kit/agents/"*.md "$wthome/.claude/agents/"
printf '\nсвоя строка\n' >> "$wthome/.claude/agents/exec-high.md"
out=$(HOME="$wthome" PATH="$gostub:$sys" python3 "$tmp/devkit-wt/tools/devkitctl/devkitctl.py" doctor -C "$mproj" 2>&1)
echo "$out" | grep -q "cp $dkreal/kit/agents/exec-high.md" || fail "сверка определения идёт не с основным чекаутом: $out"
...
out=$(HOME="$wthome" PATH="$gostub:$sys" python3 "$tmp/devkit-wt/tools/devkitctl/devkitctl.py" doctor --fix -C "$mproj" 2>&1)
echo "$out" | grep -q "cp $dkreal/kit/agents/exec-high.md" || fail "с worktree --fix потерял находку про разошедшееся определение: $out"
...
out=$(HOME="$wthome" PATH="$gostub:$wtbin:$sys" python3 "$tmp/devkit-wt/tools/devkitctl/devkitctl.py" doctor --fix -C "$mproj" 2>&1)
...
find "$rdk" -name '*.md' -not -path "$rdk/kit/skills/*" -not -path "$rdk/kit/agents/*" -delete
rm -rf "$rdk/kit/skills"/*
mkdir -p "$rdk/kit/skills/tiny-skill"
printf -- '---\nname: tiny-skill\ndescription: тестовый скилл.\n---\n%s' "$(python3 -c 'print("т" * 200, end="")')" \
    > "$rdk/kit/skills/tiny-skill/SKILL.md"
printf '# ядро\n\nтекст ядра.\n' > "$rdk/RULES.core.md"
...
mkdir -p "$rdk/docs"
printf '# Задачи\n\nПрефикс: RD\n' > "$rdk/docs/TASKS.md"
rdkctl="$rdk/tools/devkitctl/devkitctl.py"
...
cp "$rdk/kit/agents/"*.md "$rdhome/.claude/agents/"
cp -R "$rdk/kit/skills/"* "$rdhome/.claude/skills/"
rdallow=$(python3 - "$rdk/tools/devkitctl" <<'EOF'
import json, sys
sys.path.insert(0, sys.argv[1])
import perms
print(json.dumps(list(perms.MACHINE_ALLOW), ensure_ascii=False))
...
python3 -c "
open('$rdk/kit/skills/tiny-skill/SKILL.md', 'w', encoding='utf-8').write(
    '---\nname: tiny-skill\ndescription: тестовый скилл.\n---\n' + 'т' * 16500)
"
cp "$rdk/kit/skills/tiny-skill/SKILL.md" "$rdhome/.claude/skills/tiny-skill/SKILL.md"
out=$(rddoc); rc=$?
...
printf -- '---\nname: probe-agent\ndescription: %s\neffort: low\n---\n\nтело\n' "$probe_desc" \
    > "$rdk/kit/agents/probe-agent.md"
cp "$rdk/kit/agents/probe-agent.md" "$rdhome/.claude/agents/"
out=$(rddoc)
...
rm -f "$rdk/kit/agents/probe-agent.md" "$rdhome/.claude/agents/probe-agent.md"
...
cproj="$tmp/cproj"
mkdir -p "$cproj"
cslug=$(python3 - "$dk/tools/devkitctl" "$cproj" <<'EOF'
import sys
sys.path.insert(0, sys.argv[1])
import context
from pathlib import Path
...
eproj="$tmp/eproj"
mkdir -p "$eproj"
eslug=$(python3 - "$dk/tools/devkitctl" "$eproj" <<'EOF'
import sys
sys.path.insert(0, sys.argv[1])
import context
from pathlib import Path
...
realbin="$tmp/realbin"
mkdir -p "$realbin"
(cd "$dk/tools/taskctl" && go build -o "$realbin/taskctl" .) || fail "taskctl для прогона start не собрался"
(cd "$dk/tools/shipctl" && go build -o "$realbin/shipctl" .) || fail "shipctl для прогона start не собрался"
