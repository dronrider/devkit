...
def work_at_risk(devkit):
    ...
    rc, out = git(devkit, "rev-list", "--count", "HEAD", "--not", "--remotes")
    if rc == 0 and out.isdigit() and int(out) > 0:
        return ("в чекауте %s коммитов, до которых не достаёт ни одна ветка origin: %s. "
                "Перевод на тег их не удалит, а вот найти их потом будет нечем: запушить "
                "и повторить" % (devkit, out))
    return ""
