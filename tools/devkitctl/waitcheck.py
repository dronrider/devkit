...
Цель DK-902 держит три уровня ожидания, и каждый сделан своей задачей. Внутри
хода ждёт голова отметкой `agentctl wait`, а оболочка конвейера ждёт за неё и
воронкой такой проход не считает. Дольше хода ждёт запись на диске: причина
Blocked с машинным разрядом и взведённое ребро «после», их поднимают `taskctl
close` и `shipctl merge` обходом ждущих. Событие, прошедшее мимо обеих команд,
добирает тик сторожка. Тесты у каждой задачи свои, а сквозного прогона через
все утилиты разом не было: связка живёт на стыке go-утилит, python-оболочки,
git и подставного tmux, и ломается она именно там.
...
ожидания), и стенд обязан оставить машину нетронутой. Служб launchd он не
трогает вовсе: тик зовётся прямо функцией обхода, а не через демона.
...
проект и подставной выкат, и общие куски стенд зовёт у соседа, а не заводит
второй копией.
"""
import os
import re
...
# Пары окружения стенда. Уведомитель молчит: звать человека к синтетической
...
# DEVKIT_HOME называет дерево devkit, откуда лестница носителей берёт оболочку
# конвейера и профиль харнеса.
STAND_ENV = {
    "DEVKIT_NOTIFY_OFF": "1",
...
# спрашивают сценарий, ворота закрытия непустую проверку, а рассказывает о себе
# стенд сам: слова соседнего круга говорили бы тут о чужой работе.
...
def mode():
    ...
    отмечается: так её поднимает обход ждущих и тик."""
    try:
...
def main():
    ...
    if m != WAIT:
        return 0
    if seen(WAIT) <= WAITS:
        # Ход кончился машинным ожиданием. Отметку кладёт сама утилита: копия
...
def wait_cmd(proj):
    """Команда, которой голова отмечает машинное ожидание. Отметку кладёт сама
...
def woke(cmd_run, proj, heads_log, task):
    ...
    оставшаяся стоять, видна по доске, не дожидаясь срока. Голова приходит уже
    своим ходом, её стенд ждёт.
    """
...
def funnel_wait(cmd_run, proj, devkit, mode_file, client, heads_log):
    ...
    с запасом, а последним проходом паркует строку, и оболочка встаёт на этом
...
def hand_merge(cmd_run, proj, task):
    ...
    ни `close` про это не знали. Признак «слита» смотрит на коммит с ID первым
    в subject, поэтому работа едет отдельным коммитом в своей ветке.
    """
    branch = task.lower()
    ...
    # ревью синтетической ветки тут не при чём. До origin работу довезёт сам
...
def which(cmd_run, name):
    ...
    называется явно: сам стенд живёт в другом окружении, чем его подпроцессы."""
...
def tick_call(cmd_run):
    """Прогонщик команд для обхода тика: тик зовёт subprocess.run, а стенду
