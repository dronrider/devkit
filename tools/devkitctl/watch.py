...
# Суффикс «[взвод]» на строке Backlog это согласие человека на самостоятельный
...
def waiters(root, call=None, taskctl=None):
    """Обход ждущих (DK-932, DK-934): строки Blocked с причиной «слияние: <ID>»
    или «закрытие: <ID>» и взведённые строки Backlog. Поднимают их первыми
    `shipctl merge` и `taskctl close` в конце своей работы, а тик добирает
    событие, прошедшее мимо них: слияние или закрытие руками, упавший хвост,
    событие до выката утилит. Взведённой строке тик нужен и сам по себе:
    ёмкость, которой ей не хватило прошлым проходом, освобождается без всякого
    события доски.
    ...
    if not bin:
        return ["корень %s: строки ждут подъёма (%d), а бинаря taskctl нет ни в PATH, ни в "
