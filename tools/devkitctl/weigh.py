...
# Надбавка DK-941, временная. Тело board-task весит 17 646 символов, а вынос
# раздела в соседний файл со ссылкой из тела просаживает послушание: замер
# стенда дал 3 из 5 против 5 из 5 у прежней редакции на сценариях конвейера.
# Порог держится на 18 000, пока DK-976 не найдёт способ резать скилл без
# потери. Формульное значение при этом остаётся 16 000, и тест сверяет оба.
SKILL_BODY_LIMIT = 18000
RUNS = 3
...
def agent_defs(devkit):
    ...
    # его отбирает doctor: соседняя проза в kit/agents/ на машину не уезжает и в
    # системном промпте себя не описывает.
    return [p for p in sorted((Path(devkit) / "kit" / "agents").glob("*.md"))
...
def active_profile(root, devkit):
    ...
    profiles, findings = rules.enabled_harnesses(root, Path(devkit) / "kit" / "harness")
    if not profiles:
...
def pockets(root, devkit, profile):
    ...
    листинг определений kit/agents/ и листинг скиллов. Этой же суммой считается
