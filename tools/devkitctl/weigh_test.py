...
class MeasureTest(SandboxCase):
    ...
    def test_12_cut_core(self):
        ...
        # claude-code объявляет ядром, и как только RULES.core.md появляется,
        # полный текст из карманов проекта уходит. Ядро на его место не встаёт:
        # его везёт глобальная точка машины, и тонкий файл проекта без доски
        # после этого не зовёт из devkit ничего (DK-193, решение 3). У этой
        # фикстуры глобальная точка нарочно тяжёлая (полный текст симлинком), и
        # своим карманом ядро тут не появляется тоже.
        ...
        try:
            ...
            self.assertNotIn("RULES.core.md", thin,
                             "ядро общих правил выписано тонким файлом второй копией")
            self.assertNotIn("RULES.md", thin, "тонкий файл не съехал с полного текста правил")
            rc, out = self.weigh_run("--runs", "1", "--limit", "20000")
            self.assertEqual(rc, 0, "weigh не прошёл на нарезанном ядре: %s" % out)
            self.assertNotRegex(out, r"(?m)^  RULES\.md \(импорт тонкого файла",
            ...
            self.assertIn_("замер: 3 420 токенов", out,
...
class PocketsDedupTest(SandboxCase):
    ...
    должен платить за него дважды (DK-190). Мерка тут двойная: общий путь на
    диске либо совпадение имени файла с его содержимым.
    """
...
class DoctorResidencyTest(SandboxCase):
    ...
    def setUpClass(cls):
        ...
        # краснота весов не тонула в находках про отсутствующую доку. kit
        # спрашивается со съёмщиками snap/: до них кода в каталоге не было.
        write(cls.rdk / "hooks" / "README.md", "# Хуки\n")
        write(cls.rdk / "tools" / "README.md", "# Утилиты\n")
        write(cls.rdk / "kit" / "README.md", "# kit\n")
