...
func TestLoadScenariosOrderAndFilter(t *testing.T) {
	...
	if strings.Join(ids, ",") != "env,judge-file,judge-red,judge,phrase,press,reply,session-only" {
...
func TestLoadScenariosForFile(t *testing.T) {
	...
	if strings.Join(ids, ",") != "env,judge-file,judge-red,judge,phrase,press,reply" {
