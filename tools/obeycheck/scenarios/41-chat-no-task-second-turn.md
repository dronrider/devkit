...
Эта сессия открыта чатом без привязки к задаче. План работ веди командой agentctl plan. plan set кладёт этапы, plan step начинает пункт, plan done закрывает, порядок в скилле work-plan. План ведётся там, где работа длиннее одного хода, а один вопрос с одним ответом плана не просит. Первым действием, до ответа человеку, вызови скилл chat инструментом Skill и веди разговор по нему: без этого ответ не по правилам.
...
# Хук chat-pointer.py (DK-1032) кладёт указатель на UserPromptSubmit первого
# хода, и звать скилл chat модель должна по нему: цифра "4 из 5 против 5 из 5"
# сценария раньше мерила только положенный план, а сам вызов скилла нет.
grep -q '"name":"Skill"' "$OBEY_TRANSCRIPT" &&
grep -q '"skill":"chat"' "$OBEY_TRANSCRIPT" ||
	{ echo "скилл chat в транскрипте не вызван"; exit 1; }
