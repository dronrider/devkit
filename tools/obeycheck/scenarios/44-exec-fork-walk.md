...
grep -qE '^[[:space:]]*-[[:space:]]*источник: (код|дока|доска|архитектура|возражение)' /tmp/forks.$$ ||
	{ echo "подстроки «источник: код|дока|доска|архитектура|возражение» в перечне развилок нет"; exit 1; }
