...
# Голова несёт свой sha, когда HEAD читается (DK-812), поэтому хвост «до
# <sha>» перед точкой необязателен, но допустим.
grep -qE '^- Вердикт: без замечаний( до [0-9a-f]{4,40})?\.' "$f" ||
	{ echo "чистого вердикта в файле нет"; exit 1; }
n=$(grep -cE '^- Вердикт: без замечаний( до [0-9a-f]{4,40})?\.' "$f")
