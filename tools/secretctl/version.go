...
const toolName = "secretctl"

// printVersion разбирает --version до всех прочих аргументов: флагу не нужны ни
// проект, ни доска, и в сеть он не ходит. Ищется он не только в первой позиции,
// как -h у соседней helpRequested: версия не должна зависеть от того, что
// написано рядом. Формат строки один на все утилиты devkit, потому что читает
// её не только человек: doctor берёт отсюда коммит бинаря. Печатается она
// первой и единственной строкой.
func printVersion(args []string, w io.Writer) bool {
	for _, a := range args {
		// Голое «--» останавливает разбор: дальше идут не наши флаги, а чужая
		// полезная нагрузка. У exec там лежит команда подпроцесса, и --version
		// внутри неё принадлежит ей; напечатать вместо прогона версию и выйти
		// нулём значит соврать обвязке, что секрет подставился.
		if a == "--" {
			break
		}
