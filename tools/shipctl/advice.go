...
// поменялся. Признак это заголовок «Сценарий проверки» в файле задачи вне
// ограждённых блоков, файл читается в дереве ветки: исполнитель и ревьювер
// пишут его туда.
...
func scenarioWarning(reviewRoot, id string, docsBranch bool) []string {
	if data, err := os.ReadFile(taskFilePath(reviewRoot, id)); err == nil {
		if hasHeading(string(data), "Сценарий проверки") {
			return nil
		}
	}
