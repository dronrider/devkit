...
// corpNextSteps называет ручной путь дальше ветки в корп-контуре: слияние и
// выкат там ведёт MR-флоу компании, а не shipctl. Текст общий для отказа
// merge/ship/revert и для подсказки на конце shipctl start (DK-123): второе
// место сочиняет тот же шаг заново, если текст не вынесен сюда одним куском.
const corpNextSteps = "trackctl submit, пуш ветки, MR, дальше по процессу контура"
...
func corpRefused(cmd string) error {
	return fmt.Errorf("корп-контур: %s здесь не работает, слияние и выкат ведёт MR-флоу компании; дальше руками: %s", cmd, corpNextSteps)
}
