...
// tools/trackctl/README.md (contour, key, branch), а боковая директория остаётся
...
func corpPipeline(t *testing.T, id string, tr corpTrack, mode trackMode) (clone, local, taskLog, trackLog string) {
	...
	// tools/trackctl/README.md.
	var tb strings.Builder
	tb.WriteString("# Привязка проекта к трекеру (tools/trackctl/README.md). Ключ repo кладёт\n")
