...
// Окно редактора на второй подписке. Ключи подписки (endpoint, токен, модель)
// лежат в настройках самой копии окна (.claude/settings.local.json). Клиент
// claude в окне ходит по ним, а не по переменной окружения: окно, открытое
// хоть из shipctl, хоть из дока, хоть через «Open Recent», уходит в endpoint
// второй подписки одинаково. CLAUDE_CONFIG_DIR тут ни при чём: эта переменная
// смотрит на каталог конфига машинного слоя и участвует не в самом окне, а в
// раскладке машинного хозяйства и в окружении подпроцесса делегирования.
