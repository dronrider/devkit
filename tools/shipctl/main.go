...
const usageText = `shipctl: слияние и откат задач по правилам доски (RULES.board.md)
  ...
  ship [--deploy "cmd"] [--push]  выкат поезда: один деплой на все слитые
        [--drain]                 после прошлого выката задачи, все разом в
                                  ...
                                  пустой поезд, занятая очередь и сломанный
                                  прод молчат нулём вместо ошибки, а провал
...
func main() {
	...
	case "ship":
		...
		fs.BoolVar(&p.Drain, "drain", false, "разлив: пустой поезд, занятая очередь и сломанный прод молчат нулём, провал деплоя ставит признак провала")
