...
//
// Он же снимает с прогона ID живой сессии (DK-631). main зовёт touchWork до
// разбора аргументов, и тесты вида goRunShip гоняют собранный shipctl
// отдельным процессом: подпроцесс наследует окружение целиком, и прогон из
// живого разговора дописывал бы реестр сессии настоящими строками «shipctl
// start» по фикстурным ID, хотя команда тут же падает на «нет на доске»
// (живой случай, найден сверкой реестра до и после go test). Без ID отметка
// работы не пишется никуда: mark() в internal/sessions выходит по пустому
// sid раньше, чем тронет диск.
