...
// acceptSufRe разбирает суффикс «[приёмка: agent|mixed| user]». Сама регулярка
// живёт в общем пакете accept, её зовёт и shipctl для совета после выката;
// здесь на неё ссылается splitTitle, снимая суффикс при разборе заголовка.
var acceptSufRe = accept.SufRe
...
// acceptOf читает вид из заголовка строки через общий разбор пакета accept:
// регулярка и снятие хвостовых суффиксов живут там однажды, и shipctl читает
// вид тем же путём для совета после выката (DK-302).
func acceptOf(title string) string {
	return accept.KindOf(title)
}
