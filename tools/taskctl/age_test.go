...
func TestCheckMarks(t *testing.T) {
	...
	cases := []struct {
		...
		// Отметка smoke называется словами (DK-947): по ней оболочка конвейера
		// отличает строку, которую ждёт человек, от строки, которую ждёт
		// проверяющий.
		{"XR-010", "Со сценарием агента и выкатом", true, "код слит, вид agent, без отметки smoke"},
		{"XR-011", "Пользовательская без выката [приёмка: user]", false, "без выката, вид user"},
		{"XR-012", "Без выката", false, "без выката, вид agent"},
		{"XR-013", "Пользовательская с выкатом [приёмка: user]", true, "код слит, вид user, без отметки smoke"},
		{"XR-014", "Прогнанная после выката", true, "код слит, вид agent, smoke прогнан"},
