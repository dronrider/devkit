...
// а общий хвост тех, кто спрашивает человека. Печать блока вопроса
// `decide --chat` зовёт его в сессии панели дашборда, а хук PreToolUse на
// AskUserQuestion (hooks/ask-panel.py) пока зовёт его по старому пути. Дело
// писателя одно: положить вопрос файлом признака, припарковать строку и
// уведомить человека.
//
// Ждать ответа тут больше нечему: до DK-724 сессии умирали с концом хода, и
// headless-проходу приходилось сидеть в Bash и опрашивать вход сам, а живая
// сессия оставляет признак лежать без срока до самого ответа. Решение целиком
// в docs/lld/DK-430-task-chat.md, решение 3, и в docs/lld/DK-756-foreign-review.md,
// решение 4.
...
type AskParams struct {
	...
	Draft    bool
	Stdin    io.Reader
	// Pack это пачка, которую собирает сам зовущий. Так печать блока вопроса
	// (`decide --chat`) передаёт писателю уже разобранный перечень развилок.
	Pack []chat.Question
