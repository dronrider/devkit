...
// них ожидается по первой непрошедшей проверке. XR-115 и XR-116 годны по всем
// критериям, кроме вида приёмки: в поезд идут только агентские задачи.
...
| XR-115 | Пользовательская [приёмка: user] | task | P3 | 6 (0+0+0+0+6) | S | - |
| XR-116 | Смешанная [приёмка: mixed] | task | P3 | 5 (0+0+0+0+5) | S | - |
...
// TestBatchAcceptKind: в поезд идут агентские задачи, а смешанный и
// пользовательский вид отсеиваются отдельной группой. XR-202 и XR-203 годны
// по цене, неопределённости и зависимостям, на старом коде (без фильтра по
// виду) они бы попали в поезд; теперь их держит вид приёмки.
func TestBatchAcceptKind(t *testing.T) {
...
| ID | Задача | Тип | P | R | Цена | Ссылка |
|--------|--------|-----|---|---|------|--------|
| XR-201 | Агентская | task | P3 | 10 (0+5+0+0+5) | S | - |
| XR-202 | Пользовательская [приёмка: user] | task | P3 | 9 (0+4+0+0+5) | S | - |
| XR-203 | Смешанная [приёмка: mixed] | task | P3 | 8 (0+3+0+0+5) | S | - |
...
`
	...
	want := strings.Join([]string{
		"batch: XR-201",
		"взято: XR-201 (task, S, неопр. 0)",
		"вид приёмки не агентский: XR-202 (user), XR-203 (mixed)",
		batchTail,
	}, "\n")
	if out != want {
		t.Fatalf("отбор по виду приёмки:\n%s\n\nожидал:\n%s", out, want)
	}
}
