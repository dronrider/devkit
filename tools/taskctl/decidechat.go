...
// chatFormatWarn это вторая строка шапки: чем формат блока держится и что его
// ломает. Без довода требование «как есть» проигрывало общим правилам письма
// харнеса, и агент заворачивал блок в ограждение кода либо пересказывал хвост
// своими словами (DK-891 и DK-1177). Строка стоит в шапке, выше блока: ниже
// его агент уже скопировал.
const chatFormatWarn = "галочки в панели встают на строки с номерами: " +
	"тройные обратные кавычки вокруг блока и свои слова вместо хвоста " +
	"«ответ строкой:» оставляют человека без выбора"
...
// одной головой, и отвечают на неё словами. Довод равенства (DK-974) стоит
// строкой под головой: рекомендованного среди вариантов нет, и человек
// читает, почему. У развилки составов кейсов в строке варианта стоит только
// голова состава, ширина и счёт: весь список кейсов в строке с номером не
// читается, а стоит он ниже раскладкой (DK-969).
func chatForkBlock(f taskform.Fork) string {
	out := []string{fmt.Sprintf("«%s»: %s", f.Name, f.Question)}
	if f.Tie != "" {
		out = append(out, "варианты равны: "+f.Tie)
	}
