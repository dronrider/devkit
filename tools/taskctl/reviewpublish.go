...
// gh есть на любой машине разработчика. Токен в команду не подставляется, в
// неё едет имя переменной окружения, и разворачивает его оболочка подпроцесса
// (RULES.md, «Секреты»): значение не попадает ни в журнал команд, ни в вывод
// ps.
...
// разворачивает она, а не Go. Переменная подменяется тестом, живого трекера
// прогону не нужно.
