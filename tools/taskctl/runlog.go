...
// logRun дописывает строку о запуске в журнал .devkit/log проекта: из него
// складывается статистика использования инструментов devkit, какие команды
// реально гоняются и как часто падают. Журнал ведётся только там, где есть
// .devkit (обвязку заводит devkitctl), провал записи работу не ломает.
