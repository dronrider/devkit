...
// строках» цели DK-395. Три строки одной полосы P2: свежая S с рангом 42
// (W = 0.8, score 52.5), M после ревью с рубежом 0.89, простоявшая сутки
// (W = 0.11 + 0.55 = 0.66, score 60.6, берём первой), она же остановленная
// в середине кода с рубежом 0.35 после суток простоя (W = 0.65 + 0.55 = 1.20,
// score 33.3, уступает свежей S). Машина даёт те же числа тем же порядком.
...
`)
	...
	want := strings.Join([]string{
		"slot: XR-320",
		"P2:",
		"  1. XR-320 (M, R 40): score 60.6, W 0.66 (C 1.00, F 0.89 «ревью пройдено, слито и выкачено», V 0.55), простой 25.0 ч",
		"  2. XR-331 (S, R 42): score 52.5, W 0.80 (C 0.80, F 0.00 «строка заведена», V 0.00)",
		"  3. XR-321 (M, R 40): score 33.3, W 1.20 (C 1.00, F 0.35 «первый коммит кода», V 0.55), простой 25.0 ч, main ушёл на 1",
		"P3:",
		"  4. XR-330 (S, R 10): score 12.5, W 0.80 (C 0.80, F 0.00 «строка заведена», V 0.00)",
	}, "\n")
	if out != want {
...
`)
	...
	if !strings.Contains(out, "score 41.2, W 0.97 (C 1.00, F 0.35 «первый коммит кода», V 0.32)") {
...
`)
	...
	want := strings.Join([]string{
		"slot: XR-105",
		"P3:",
		"  1. XR-105 (S, R 12): score 15.0, W 0.80 (C 0.80, F 0.00 «строка заведена», V 0.00)",
		"отказы:",
...
`)
	...
	want := strings.Join([]string{
		"slot: XR-303",
		"P3:",
		"  1. XR-303 (S, R 9): score 11.8, W 0.76 (C 0.80, F 0.35 «первый коммит кода», V 0.24), простой 0.0 ч",
		"отказы:",
		"  потолок деревьев 0 исчерпан: XR-201, XR-202",
