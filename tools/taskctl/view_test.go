...
| XR-011 | Пользовательская проверка без выката | task | P3 | 5 (0+3+1+0+1) | S | [tasks/XR-011.md](tasks/XR-011.md) |
...
func TestListAnnotatesCheckRows(t *testing.T) {
	...
	for _, want := range []string{
		...
		"  код слит, сценарий агентский, строка не двигалась 10 дней",
		"| XR-011 | Пользовательская проверка без выката",
		"  без выката, сценарий пользовательский, строка не двигалась 10 дней",
	...
	// У строки без файла задачи меток очереди/сценария нет вовсе, только
	// возраст (у XR-012 тоже нет отдельного коммита строки, но она разбирается
	// той же логикой, что и остальные Check-строки, поэтому возраст есть).
	if got := lineAfter(t, out, "| XR-012 | Без файла задачи"); strings.Contains(got, "код слит") || strings.Contains(got, "без выката") {
		t.Fatalf("у задачи без файла не должно быть меток очереди/сценария: %q", got)
	}
...
func TestListNoAnnotationsWhenDirty(t *testing.T) {
	...
	if !strings.Contains(out, "код слит, сценарий агентский") {
...
func TestShowAnnotatesCheckRow(t *testing.T) {
	...
	for _, want := range []string{
		"XR-010 в check",
		"код слит, сценарий агентский, строка не двигалась 1 день",
	} {
		if !strings.Contains(out, want) {
